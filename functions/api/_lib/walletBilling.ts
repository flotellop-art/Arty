...
export async function beginWalletBilling(
  ...
  params: { email: string; model: string; maxOutputTokens: number | undefined },
): Promise<WalletBillingStart> {
  const { email, model, maxOutputTokens } = params
  ...
  const estMicro = estimateReserveMicro(model, maxOutputTokens)
