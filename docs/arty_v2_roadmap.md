...
## 💡 IDÉES POST-OPTION A À TRIER (feedbacks bêta)

Section vivante pour capturer les retours utilisateurs avant de décider si on les intègre à Option A, à la V2.1, ou plus tard.

### #24. Notifications proactives agenda

**Priorité** : P1 (à confirmer) · **Effort** : S (2-3h) · **Impact** : 🟢
**Source** : Mégane (VIP tester Firebase, 21 avril 2026)

**Description** : notification native Android 15 min avant chaque événement Google Calendar synchronisé, **même si l'app est fermée**.

**Pain identifié** : *"il y a tellement de choses à noter dans mon agenda Google que ça peut passer un petit peu à la trappe. Ce serait bien que l'appli puisse envoyer une notification pour attirer un petit peu l'attention."*

**État actuel du code** :
- `src/services/notificationService.ts` expose déjà `notifyCalendarEvent()` mais personne ne l'appelle
- Mécanisme de programmation basé sur le Service Worker / `setTimeout` → ne tient pas quand l'app Capacitor est fermée/tuée par Android

**Stack technique** : `@capacitor/local-notifications` (plugin officiel Capacitor) qui utilise `AlarmManager` Android. La notif est gérée par l'OS, pas par le WebView → fire même app fermée.

**Implémentation** :
1. `npm install @capacitor/local-notifications` + `npx cap sync android`
2. Remplacer `scheduleNotification()` dans `notificationService.ts` par l'appel natif
3. Dans `useGoogleCalendar` (ou équivalent qui fetch l'agenda) : `forEach event → LocalNotifications.schedule(...)`
4. Permission notif Android 13+ déjà gérée dans `AndroidManifest.xml` (BUG 33)

**Limite acceptée** : user doit ouvrir Arty au moins une fois tous les 3-7 jours pour re-sync les futurs événements. Pour un MVP, OK. Phase 2+ : background fetch périodique via `@capacitor/background-runner`.

**Justification P1** :
- Pain réel d'une VIP tester (signal fort)
- Petit effort (2-3h)
- Débloque la même plomberie pour les notifs proactives suivantes (emails urgents, intentions morning brief, tâches en retard)
- Différenciateur Product Hunt (*"Arty te prévient au bon moment, pas juste un calendrier de plus"*)

**Décision à prendre** : intégrer à l'Option A (push 1.0.15 avant launch v2) ou reporter à V2.1 ?
