...
| C12 | (nouveau, revue produit C1) **Copy « 80 Gemini Pro » structurellement mensonger** | ✅ **FAIT (18/07, PR à suivre)** — copy retiré des 5 clés fr/en (upsell 218-219 → « Gemini » tout court, subDesc/a1/subscriptionDescription → « 150 Sonnet/Opus + 100 GPT-5 ») + lp/prix ; le bucket est masqué DANS LA RÉPONSE `subscription/status` (`hiddenBuckets`) → disparaît aussi des vieux APK, `unknown-model` (filet technique affiché en brut dans le tooltip) masqué au passage ; **le cap serveur reste enforcé** (checkPremiumCap intact, défense en profondeur si un client forge un `-pro`). Garde anti-dérive `c12GeminiProCopy.test.ts` : « Gemini Pro » dans locales/lp OU retrait du filtre → CI rouge. Réintroduction copy+bucket UNIQUEMENT avec un vrai chemin (GA 3.5 Pro) | S | — |
