...
**Version :** 1.4.2 — cinquième verdict du décideur intégré (13 juillet 2026)<br>
**Date :** 13 juillet 2026<br>
**Statut :** architecture hors CASA validée ; CDC traité comme un **contrat de livraison** — chaque PR passe seule, chaque garantie a un test observable. **Implémentation EN PAUSE** jusqu'au GO sur cette v1.4.2.<br>
...
- **Navigation « onglet réservé »** (D15, complété par D24) : le code actuel `window.open(url, '_blank', 'noopener,noreferrer')` retourne `null` MÊME en cas de succès → double navigation possible avec le repli `location.assign()`. Un simple lien `<a>` corrigerait la double navigation mais ne garantit PAS que la **copie presse-papiers asynchrone** soit terminée avant l'ouverture. Correctif retenu : (1) **valider et construire l'URL AVANT toute réservation d'onglet** — si la re-validation échoue, aucun onglet n'est ouvert (message `needs_details`) ; (2) au clic (synchrone, dans le geste utilisateur), **réserver un onglet vide** `const tab = window.open('', '_blank')` (sans `noopener` dans les features pour obtenir la référence, puis `tab.opener = null`) ; (3) `await` la copie ; (4) **naviguer l'onglet réservé** (`tab.location.href = url`). Cas d'échec (D24/D27/D34 — **jamais de no-op silencieux**) : popup bloqué (`tab === null`) → la carte affiche « **Requête copiée** » + un **bouton Gmail manuel** (lien direct actionnable par l'utilisateur, le geste utilisateur du clic n'étant pas bloquable) ; **copie refusée/échouée → fermer l'onglet vide réservé** (`tab.close()`) + message explicite avec la requête affichée à copier manuellement ; **onglet fermé par l'utilisateur avant navigation** → même état « Requête copiée » + bouton Gmail manuel ; **échec de la navigation elle-même** (`tab.location.href = url` lève) → `try/catch`, fermeture de l'onglet + même état actionnable ; **`encodeURIComponent` peut lever `URIError`** (surrogate isolé dans une requête éditée — la leçon « surrogate pairs » de PR #109-113) → `try/catch` dès la construction, échec → `needs_details`, aucun onglet ouvert. JAMAIS de `location.assign()` automatique sur l'onglet courant. **Ce durcissement s'applique à TOUS les parcours web (D24)** : flag ON (deep link), flag OFF et mobile web (ouverture de la racine `GMAIL_HOME_URL` via le même mécanisme onglet-réservé — l'implémentation actuelle de `openGmailHome()` a le même bug `noopener`/double navigation).
...
|---|---|---|---|---|---|
| `functions/api/drive/action.ts` (réactivé, opérations réduites : `list`, `read`, `download`, `create` — la « recherche » est `list`+`q`, l'« export » vit dans `read`/`download`) | **DEUX justificatifs (D13)** : identité/session Arty (header existant) + jeton Drive contextuel (`x-drive-token`) ; `tokeninfo` sur les deux : jeton Drive == **exactement `drive.file`** (D12), identité en allowlist exacte des scopes publics (D20), **`aud` + `azp` validés séparément contre les couples Web/Android autorisés** (`aud` valide + `azp` étranger → 403), **`sub` obligatoire et identique** entre les deux jetons, expiration/claims en échec fermé (D25, §4.0 P0-a) | le token de l'utilisateur EST l'autorité — Google filtre le périmètre du scope ; le **registre** filtre en plus côté app (D4) ; IDs validés regex (BUG 32) ; terme `fullText` échappé ; `X-Goog-Drive-Resource-Keys` uniquement pour les ids du registre (D5) | pas de clé owner dépensée (API Drive gratuite) ; quotas/caps par appel conservés (bornes précises ci-dessous) | erreurs Google masquées (générique + `console.error`, pattern N-2/PR #307) | Origin whitelist stricte inchangée ; **listes `Access-Control-Allow-Headers` étendues à `x-drive-token`** (`:92` + `:127`, D20) |
...
| ID | Exigence |
|---|---|
| GML-P1-08a | Lien direct résultats Gmail web desktop sous flag `VITE_GMAIL_SEARCH_DIRECT_LINK` **indépendant du profil Phase 0**, navigation « onglet réservé » sur TOUT le web (flag ON/OFF, desktop + mobile web) : copie terminée AVANT navigation, un seul onglet, jamais de `location.assign` automatique ; échecs actionnables (« Requête copiée » + bouton Gmail manuel) pour popup bloquée, copie refusée, onglet fermé, navigation impossible, `URIError` (D11/D15/D24/D34) |
...
Chrome/Firefox/Safari desktop × {1 compte, multi-comptes, non connecté} × {flag ON, flag OFF} + mobile web (Chrome Android, Safari iOS — racine Gmail via l'ouvreur commun) : le lien ouvre les bons résultats ou échoue vers l'état actionnable « Requête copiée + bouton Gmail manuel ». Cas d'échec à couvrir explicitement (D34) : popup bloquée, copie refusée, onglet fermé avant navigation, navigation impossible, `URIError`. Vérifier l'ABSENCE de double navigation dans tous les cas. Annexer la matrice à la PR.
...
- Contrôle serveur à deux justificatifs (D25) : jeton Drive avec scope surnuméraire → 403 ; `drive.file` manquant → 403 ; jeton d'identité hors allowlist exacte des scopes publics → 403 ; couple `aud`/`azp` hors couples autorisés → 403 ; **`aud` valide + `azp` étranger → 403 (test canonique)** ; **`sub` divergent entre les deux jetons → 403 ; `sub` ABSENT sur l'un des jetons → 403 (échec fermé)** ; jeton expiré → 403 (mocks tokeninfo, pattern `audValidation.test.ts`).
- Ouvreur web : `encodeURIComponent` qui lève `URIError` (surrogate isolé) → `needs_details`, aucun onglet ; échec d'assignation `tab.location.href` → onglet fermé + état « Requête copiée » + bouton Gmail manuel ; popup bloquée / onglet fermé → même état actionnable, JAMAIS de no-op (D34).
...
1. Sur web desktop, flag ON : « retrouve le mail de Paul sur le devis de juin » → carte avec requête → un clic → **la copie est terminée avant la navigation**, Gmail ouvert **sur les résultats**, **un seul onglet** ouvert. Mêmes garanties flag OFF et sur mobile web (racine Gmail). Tout échec (popup bloquée, copie refusée, onglet fermé, navigation impossible, `URIError`) aboutit à l'état actionnable « Requête copiée » + bouton Gmail manuel — jamais un silence.
...
| **PR-B0** | **Retrait ATOMIQUE de Drive seul des tombstones (D29) + protections D3/D12/D13/D25/D30 : contrôle serveur à deux justificatifs (`sub` obligatoire, tuples `(azp, aud)` réels figés par `GATE-TOKEN-PAIRS-01` §8.0, allowlist scopes, expiration fail-closed) + `x-drive-token` au CORS (les 2 listes) + bornes précises du proxy avec tests de frontières (D33) + révocation/migration anciens grants + preuve growth-orchestrator** | 2,5-3 j + ops |
| PR-B1 | Autorisation contextuelle web (GIS `include_granted_scopes: false`, `GIS_ALLOWED_ORIGINS` exactes + test `origin_mismatch` D32, CSP accounts.google.com) + état/CTA « Reconnecter mes fichiers » (D19) + amendements nominatifs RÈGLES 1/4 + `vite-env.d.ts` + `.env.example` + doc déploiement (D32) + tests parité (TS + `.java`) | 1,5-2 j |
