...
| I3 | **`oauth_profile` DIFFÉRÉ en PR-B0** (question posée, réponse : « Plus tard, en PR-B0 ») : aucun lecteur n'existe avant le contrôle serveur B0, et le champ sans consommateur rejouerait la classe BUG 51. D22 se réduit en PR-0 au fix `storeTokens`/`storeUser` du login natif. |
...
**P0-a-bis — Stockage des jetons natifs conforme (D22/I3).** PR-0 fait passer `src/App.tsx` (`onNativeGoogleLogin`) par `storeTokens`/`storeUser`, supprimant les deux écritures `setJSON` brutes et rétablissant le stockage chiffré. Le champ `oauth_profile` reste volontairement **différé en PR-B0**, quand son premier lecteur serveur existera. Le `refresh_token` de ce parcours est frais via `requestServerAuthCode`/l'échange de code ; sa préservation n'est pas une responsabilité de `storeTokens`.
...
|---|---|---|
| **PR-0** | **Fondation construite from scratch (I1/D29)** : scanner sémantique `check-public-google-access.mjs` (scopes publics exacts, URL Gmail légitime non confondue avec un scope), inspections du manifest mergé + APK debug et inspection AAB dans le workflow release, renommages internes `PUBLIC_GOOGLE_*`, migration de `App.tsx` vers `storeTokens`/`storeUser` **sans `oauth_profile`** (différé en PR-B0, I3), exclusion Sheets explicite, **quatre connecteurs en tombstones HTTP 410**, variable de rollback serveur documentée et suite entièrement verte (`no-casa:check` inclus). Prérequis de TOUTES les PRs suivantes | 2-2,5 j (décideur + Claude) |
