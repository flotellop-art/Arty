...
**Version :** 1.4.3 — sixième verdict du décideur intégré (13 juillet 2026)<br>
**Date :** 13 juillet 2026<br>
**Statut :** architecture hors CASA validée ; GO sans réserve donné sur la v1.4.3 le 13 juillet 2026. **Implémentation démarrée** : PR-0 (#338) au gate pré-merge ; les PR suivantes restent ordonnées après son merge.<br>
...
| D20 | **(⚠️ champs corrigés par D25 — le fond reste valable)** **Double contrôle serveur précis** : ~~`tokeninfo` d'un access token documente `user_id`, `audience`/`aud`, `issued_to`~~ (erreur factuelle : le `tokeninfo` moderne renvoie `sub`/`aud`/`azp`, voir D25). Comparer l'**identité stable réellement documentée** entre jeton web et jeton Android, **échec fermé** si le champ manque. Ajouter **`x-drive-token` aux DEUX listes `Access-Control-Allow-Headers`** du middleware (`functions/api/_middleware.ts:92` préflight ET `:127` réponse — vérifié absent) sinon le web échoue au préflight avant d'atteindre le proxy. Jeton principal : **allowlist exacte des scopes publics** au lieu d'une denylist partielle. |
...
**Quatrième verdict (v1.4.1) — NO-GO v1.4, GO maintenu sur l'architecture hors CASA :**

| # | Décision actée (v1.4.1) |
|---|---|
| D25 | **Correction factuelle de D20** : le `tokeninfo` moderne utilisé par Arty renvoie **`sub`, `aud`, `azp`** — PAS `user_id`/`audience`/`issued_to`. Contrôle corrigé : (i) comparer le **`sub` OBLIGATOIRE** des deux jetons (absent → 403, échec fermé) ; (ii) valider **séparément `azp` ET `aud`** contre des **couples Web/Android explicitement autorisés** ; (iii) vérifier scopes, expiration et claims en échec fermé ; (iv) test canonique : **`aud` valide + `azp` étranger → 403**. Source : Google Cloud — User access tokens. |
| D26 | **PR-0 = vraie PR de stabilisation**, pas une « synchronisation » : la migration locale compte **31 fichiers modifiés + 5 nouveaux**, son `no-casa:check` **échoue encore sur l'URL Gmail légitime**, un test **importe le module supprimé `gmailNoCasaPhase0`**, et `App.tsx` stocke toujours les jetons natifs sans `oauth_profile`. PR-0 livre : scanner sémantique, renommages complets, tests réparés, `storeTokens`/`storeUser`, **suite entièrement verte**. Drive/Gmail/Contacts/Sheets restent DÉSACTIVÉS dans PR-0 ; **PR-B0 réactive uniquement Drive** avec ses protections. |
| D27 | **Propagation intégrale de D19-D24 dans tout le document** : mobile web passe sur le nouvel ouvreur (plus d'`openGmailHome()` ancien), plus aucun `setJSON`/« middleware inchangé »/« renouvellement silencieux » résiduel, critères d'identité alignés sur D25, **valeurs précises** des bornes du proxy + allowlist MIME explicite, traitement des échecs de navigation et d'`encodeURIComponent`, et exigence de test des **opérateurs compilés** : « Le devis que Paul m'a envoyé » DOIT produire `from:Paul`. |
| D28 | **D23 tranché : alias preview FIXE retenu** (recommandation du décideur — conserve les tests navigateur avec des origines OAuth exactes). |
| — | **Ordre corrigé : PR-0 → A1 → A2 → B0 → B1 → B2 → spike fullText → B3 → B4** (le spike a besoin de fichiers accordés via Picker, donc après B2). |

**Cinquième verdict (v1.4.2) — NO-GO documentaire sur `ca85901`, corrections circonscrites :**

| # | Décision actée (v1.4.2) |
|---|---|
| D29 | **PR-0 doit pouvoir être verte seule** : les trois scanners APK/AAB sémantiques sont DÉPLACÉS de PR-A1 vers PR-0 (ils sont introduits par la migration et rejetteraient immédiatement `mail.google.com`). PR-0 se termine avec les **quatre connecteurs (Drive/Gmail/Contacts/Sheets) en tombstones HTTP 410** et TOUS les contrôles verts. PR-B0 retire **atomiquement Drive seul** de la liste des tombstones ; Gmail/Contacts/Sheets restent en 410. |
| D30 | **Tuples `(azp, aud)` ORDONNÉS et RÉELS** pour les quatre types de jetons — identité Web, Drive Web, identité Android, Drive Android — établis par un **gate sur de vrais jetons** (`GATE-TOKEN-PAIRS-01`, §8.0) AVANT de figer l'allowlist. **Pas de produit cartésien** entre une liste d'`azp` et une liste d'`aud`. |
| D31 | **Changement de compte** : suppression explicite de `drive-connected-files-enc` + reset du cache Drive déchiffré **AVANT** `setActiveSession()`/`clearActiveSession()` — `clearActiveKeys()` ne réalise PAS cette purge (ne pas le supposer). |
| D32 | **Origins exactes** (schéma + port réels — `https://localhost` sans port ne suffit pas), test `origin_mismatch`, vérification que GIS est masqué sur les previews dynamiques. **L'exception clé Picker modifie nominativement les RÈGLES 1 ET 4 de `CLAUDE.md`, `src/vite-env.d.ts`, `.env.example` et la documentation de déploiement** — pas seulement la RÈGLE 1. |
| D33 | **Bornes complétées** : plafond sur le **corps brut** de `create` ; chaînes **MIME canoniques énumérées** (pas « OOXML » ni « Google Docs ») ; `resourceKey` = **chaîne opaque bornée** — rejeter contrôles, CR/LF, `/` et `,` (délimiteurs du header), SANS imposer la regex non documentée `[A-Za-z0-9_-]+` ; tester chaque frontière : **limite−1, limite, limite+1 et timeout**. |
| D34 | **Fin de propagation Gmail** : popup bloquée ou onglet fermé → afficher « requête copiée » + **bouton Gmail manuel** (jamais de no-op silencieux) ; GML-P1-08a, matrice §8.3 et critères d'acceptation étendus à flag OFF, mobile web, copie refusée, onglet fermé, navigation impossible, `URIError`. §3.1 corrigé : le validateur **rejette** les bidi/contrôles (il ne les supprime pas) et l'URL contient aussi l'`authuser` validé. Invariant reformulé : « aucun scope restreint dans le **projet OAuth du client public** » (cohérent avec le `growth-orchestrator` séparé). |

**Sixième verdict (v1.4.3) — trois retouches mécaniques avant GO ; D29/D31/D33 et l'invariant OAuth public sont VALIDÉS :**

| # | Décision actée (v1.4.3) |
|---|---|
| D35 | **Restrictions de la clé Picker alignées sur D32** : referrers COMPLETS avec variantes `/*` — `https://tryarty.com/*`, `https://preview.tryarty.com/*`, `http://localhost:5173/*`. **Retirés** : `*.appfacade.pages.dev` (GIS/Picker masqués sur previews dynamiques), `capacitor://localhost` et `https://localhost` (le Picker web n'y tourne pas — Android passe par le parcours natif D2). **`www.tryarty.com` sert actuellement Arty sans redirection** : décision = **redirection 301 vers le domaine canonique `tryarty.com`** (livrable PR-B1, règle Cloudflare) — à défaut, autoriser `https://www.tryarty.com/*` dans les referrers ET `https://www.tryarty.com` dans `GIS_ALLOWED_ORIGINS` (les deux listes, jamais une seule). **La création DNS de `preview.tryarty.com` est un livrable PR-B1.** |
| D36 | **Trois états d'échec Gmail DISTINCTS — ne pas les mélanger** : (1) copie RÉUSSIE puis popup bloquée / onglet fermé / navigation impossible → état `copied_manual` : « Requête copiée » + bouton Gmail ; (2) **copie REFUSÉE** → état `copy_error` : requête affichée **sélectionnable** + bouton Gmail — **ne jamais prétendre qu'elle est copiée** ; (3) **`URIError` AVANT ouverture** → `needs_details`, **aucun `window.open`**. Corrigé dans GML-P1-08a, la matrice §8.3, les AC, et le plan de tests (« onglet fermé → no-op » supprimé). |
| D37 | **Fermeture formelle de `GATE-TOKEN-PAIRS-01`** : le gate §8.0 entre dans les critères d'acceptation ; **test POSITIF de chacun des quatre tuples réels** en plus des rejets ; l'audit dit « **tuple ordonné exact** », plus « aud et azp validés séparément » ; un **harness Web/APK temporaire est créé en début de PR-B0** pour obtenir les vrais jetons AVANT que B1/B4 existent — sinon la séquence est circulaire. |

> ⚠️ **PR-0 — stabilisation de la migration locale (D26)** : plusieurs références des verdicts 3 et 4 (`scripts/check-public-google-access.mjs`, `npm run no-casa:check`, `PUBLIC_GOOGLE_SCOPES`/`PUBLIC_GOOGLE_OAUTH_PROFILE`/`BLOCKED_PUBLIC_GOOGLE_TOOL_NAMES`, `oauth_profile`, inspections APK/AAB) proviennent de la **migration locale du décideur (« Project Arty NoCasa Migration »), non poussée sur `origin` au 13 juillet** (31 fichiers modifiés, 5 nouveaux). Sur `origin/main`, les équivalents actuels restent `gmailNoCasaPhase0.ts`/`.test.ts` — et les points `_middleware.ts:92`/`App.tsx:957` sont, eux, **vérifiés présents sur `origin`**. PR-0 pousse ET stabilise cette migration (état cible : contrôles sémantiques qui acceptent les valeurs légitimes, aucun import de module supprimé, jetons natifs via `storeTokens`/`storeUser`, suite verte, connecteurs Google toujours désactivés).

**Décisions d'implémentation PR-0 (13 juillet 2026, ratifiées en session après le GO) :**

| # | Décision |
|---|---|
| I1 | **La « migration locale » n'existait pas** (le décideur n'avait rien codé — les relectures décrivaient un dossier d'exploration d'assistant local). PR-0 est construite **from scratch** d'après ce CDC ; l'encadré ci-dessus et le « 31 fichiers + 5 nouveaux » de D26 sont sans objet. |
| I2 | **Coupure immédiate RATIFIÉE** (question posée explicitement, réponse : « Coupure immédiate ») : les tombstones 410 sont **actifs PAR DÉFAUT** au déploiement, **avant toute auth** (uniformes, aucun aller-retour Google). `LEGACY_GOOGLE_CONNECTORS_ENABLED='true'` (env Cloudflare) + « Retry deployment » est un **rollback SERVEUR** d'urgence : il réactive les endpoints pour les anciens APK/caches, mais ne restaure pas les outils dans un bundle web déjà compilé avec `VITE_GMAIL_NO_CASA_PHASE0=true`. Un rollback produit complet exige de poser AUSSI ce flag client à `false`/absent avant le même Retry. Au 13 juillet, le flag client `true` est déjà compilé dans la Production ; la coupure serveur reste déclenchée par le merge de PR-0. |
| I3 | **Décision remplacée le 14 juillet 2026** : `oauth_profile: calendar-events-v1` est maintenant émis et vérifié par le serveur, puis stocké atomiquement avec le grant chiffré. Il sert à migrer le scope `calendar` vers `calendar.events` sans recycler un ancien refresh token. PR-B0 réutilisera ce contrat au lieu de l'introduire. |
| I4 | **Mapping des « 3 inspections » (D29)** : (1/3) scanner source sémantique `no-casa:check` (job `verify-app`) ; (2/3) manifest mergé + (3/3) `aapt dump` sur l'APK debug (job `verify-android`, builds déjà produits). L'inspection **AAB** vit dans le workflow release manuel (`android-playstore-aab.yml`, bundletool) — il ne gate jamais les PRs. Inspections Android en **denylist exacte** d'abord (permissions interdites + parité source) ; gel en allowlist stricte une fois la première base CI mergée établie. |
| I5 | **Cas double-usage `https://mail.google.com/`** : à la fois scope restreint et URL légitime du hand-off — exclu du balayage de littéraux du scanner, couvert par les contrôles d'égalité exacte des déclarations de scopes (c'était l'échec historique de `no-casa:check` sur l'URL légitime). |
| I6 | **Trou Phase 0 corrigé** : Sheets (l'un des quatre connecteurs D26) n'était pas gaté côté client — `sheetsToolDefinitions` exclu du profil public + `export_clients_to_sheets`/`export_projets_to_sheets` ajoutés à la blocklist. Le reste de l'hygiène blocklist demeure PR-A1. |
| I7 | **Renommages** : symboles TS renommés (`publicGoogleOAuthProfile.ts`, `PUBLIC_GOOGLE_SCOPES`, `BLOCKED_PUBLIC_GOOGLE_TOOL_NAMES`, `isPublicGoogleOAuthProfileEnabled`) ; les noms d'interrupteurs EXTERNES (`VITE_GMAIL_NO_CASA_PHASE0`, `ARTY_GMAIL_NO_CASA_PHASE0`/`BuildConfig.GMAIL_NO_CASA_PHASE0`) restent des alias documentés — les renommer désynchroniserait silencieusement la config Cloudflare/Gradle déployée (classe BUG 40). Renommage externe = étape ops séparée. |
...
- aucun scope restreint dans le **projet OAuth du client public** (D34 — le `growth-orchestrator` vit dans son propre projet séparé, prouvé par P0-b) ;
...
| `drive.readonly`, `drive`, `drive.metadata`, **`drive.metadata.readonly`**, `drive.activity(.readonly)`, `drive.meet.readonly`, `drive.scripts` | **Restreint** | — | **Oui** |
...
- [Google Cloud — User access tokens : réponse `tokeninfo` moderne (`sub`/`aud`/`azp`/`scope`/`expires_in`)](https://cloud.google.com/docs/authentication/token-types#user) (D20 corrigé par D25)
...
- **Plateformes (D27)** : le **deep link** est web desktop uniquement ; mais le **nouvel ouvreur « onglet réservé » remplace `openGmailHome()` sur TOUT le web** — desktop et mobile web, flag ON (URL de résultats) comme flag OFF (racine `GMAIL_HOME_URL`). Seul le natif Capacitor conserve `Browser.open()` (plugin système, pas de `window.open`).
...
- **Navigation « onglet réservé »** (D15, complété par D24) : le code actuel `window.open(url, '_blank', 'noopener,noreferrer')` retourne `null` MÊME en cas de succès → double navigation possible avec le repli `location.assign()`. Un simple lien `<a>` corrigerait la double navigation mais ne garantit PAS que la **copie presse-papiers asynchrone** soit terminée avant l'ouverture. Correctif retenu : (1) **valider et construire l'URL AVANT toute réservation d'onglet** — si la re-validation échoue, aucun onglet n'est ouvert (message `needs_details`) ; (2) au clic (synchrone, dans le geste utilisateur), **réserver un onglet vide** `const tab = window.open('', '_blank')` (sans `noopener` dans les features pour obtenir la référence, puis `tab.opener = null`) ; (3) `await` la copie ; (4) **naviguer l'onglet réservé** (`tab.location.href = url`). Cas d'échec — **TROIS états distincts, jamais mélangés, jamais de no-op silencieux (D36)** :
  1. **`needs_details`** — `URIError` d'`encodeURIComponent` (surrogate isolé dans une requête éditée — leçon « surrogate pairs » de PR #109-113) ou re-validation en échec, détecté à la CONSTRUCTION de l'URL, donc **AVANT toute réservation : aucun `window.open`**.
  2. **`copied_manual`** — la copie a RÉUSSI mais l'ouverture échoue (popup bloquée `tab === null`, onglet fermé par l'utilisateur avant navigation, ou `tab.location.href = url` qui lève → `try/catch` + `tab.close()`) : la carte affiche « **Requête copiée** » + un **bouton Gmail manuel** (le geste utilisateur du clic n'étant pas bloquable).
  3. **`copy_error`** — la copie presse-papiers est REFUSÉE/échoue : fermer l'onglet vide réservé (`tab.close()`), afficher la requête **sélectionnable** + le bouton Gmail manuel — **ne JAMAIS prétendre que la requête est copiée**.

  JAMAIS de `location.assign()` automatique sur l'onglet courant. **Ce durcissement s'applique à TOUS les parcours web (D24)** : flag ON (deep link), flag OFF et mobile web (ouverture de la racine `GMAIL_HOME_URL` via le même mécanisme onglet-réservé — l'implémentation actuelle de `openGmailHome()` a le même bug `noopener`/double navigation).
...
- **Construction de l'URL** (D16, corrigé par D34) : la requête affichée dans la carte est **éditable par l'utilisateur** — la valeur naviguée n'est donc PAS garantie par la compilation amont. Garantie réelle, dans l'ordre, au moment du clic : (1) **re-validation** de la valeur finale par `validateGmailSearchQuery` — qui **REJETTE** (retourne `false`, ne supprime rien) : bidi/contrôles présents, longueur > 500, URLs, `/u/<n>/`, paramètres `q|query|token|callback`, opérateurs hors allowlist, emails absents de la demande source ; (2) **`encodeURIComponent`** de la requête validée, qui neutralise tout caractère de structure d'URL (`#`, `/`, `?`) dans le fragment. Échec de re-validation → pas de navigation, message `needs_details`. L'URL ne contient QUE la requête encodée et, le cas échéant, l'**`authuser` validé** (§ Compte ci-dessus).
...
- **Tests** : panel FR/EN d'au moins 20 phrasings positifs (directs + indirects), 10 négatifs généraux (dont colis/SMS/WhatsApp/« message d'accueil », D17) et 5 négatifs Drive ; chaque cas raté remonté en usage = un pattern ajouté + un test de non-régression (règle BUG 56). **Les cas positifs assertent les OPÉRATEURS COMPILÉS, pas seulement le statut (D27)** : « Le devis que Paul m'a envoyé en juin » DOIT produire `from:Paul` (+ bornes de dates de juin) — un `ready` avec une requête vide ou hors-sujet est un échec de test.
...
- **Périmètre réel des contrôles (D21)** : la réécriture sémantique ne se limite PAS aux deux tests unitaires ci-dessus. PR-0 livre le scanner `scripts/check-public-google-access.mjs`, les inspections du manifest mergé et de l'APK debug, ainsi que l'inspection AAB du workflow release. Le scanner compare les déclarations publiques par ensembles exacts, autorise le littéral non-sensible `drive.file` hors login et distingue l'URL Gmail légitime du scope `mail.google.com`. Même principe partout : identifiants exacts, jamais de matching de scopes par sous-chaîne.
- **Renommages (D21)** : PR-0 remplace les références internes `gmailNoCasaPhase0` par `PUBLIC_GOOGLE_SCOPES`, `PUBLIC_GOOGLE_OAUTH_PROFILE` et `BLOCKED_PUBLIC_GOOGLE_TOOL_NAMES`. Les noms des interrupteurs externes restent provisoirement stables conformément à I7.
...
1. **Contrôle serveur à DEUX justificatifs (D13, précisé par D20)** : le proxy Drive ne reçoit pas seulement le jeton Drive — chaque requête porte : (a) l'**identité/session Arty** (jeton Google du login principal, header existant) et (b) le **jeton Drive contextuel** (header dédié `x-drive-token` — à ajouter aux DEUX listes `Access-Control-Allow-Headers` du middleware, `_middleware.ts:92` préflight ET `:127` réponse, vérifié absent aujourd'hui : sans ça le web échoue au préflight). Le serveur valide les deux via `tokeninfo` (cache court) et exige : (i) jeton Drive : `scope` == **exactement `drive.file`**, sans aucun scope supplémentaire (D12) ; (ii) jeton d'identité : **allowlist exacte des scopes publics** (l'ensemble du login §5.1, comparaison stricte — PAS une denylist partielle, D20) ; (iii) validation de chaque jeton contre des **tuples `(azp, aud)` ORDONNÉS et RÉELS** (D30) — un tuple par type de jeton : identité Web, Drive Web, identité Android, Drive Android — constante `AUTHORIZED_CLIENT_PAIRS` figée UNIQUEMENT après `GATE-TOKEN-PAIRS-01` (§8.0, tokeninfo sur de vrais jetons des deux plateformes) ; **JAMAIS de produit cartésien** entre une liste d'`azp` et une liste d'`aud` — cas canonique refusé : **`aud` valide + `azp` étranger → 403** (D25) ; (iv) **`sub` OBLIGATOIRE et identique entre les deux jetons** — le `tokeninfo` moderne renvoie `sub`, `aud`, `azp` (D25) ; `sub` absent sur l'un des jetons → **403, échec fermé** — sinon un utilisateur connecté à Arty avec le compte A pourrait connecter le Drive du compte B ; (v) expiration (`expires_in > 0`) et cohérence des claims vérifiées en échec fermé. Tout jeton non conforme → 403 avec message de reconnexion actionnable. Ce contrôle force mécaniquement la migration des anciens jetons.
...
**P0-a-bis — Stockage des jetons natifs conforme (D22/I3).** PR-0 fait passer `src/App.tsx` (`onNativeGoogleLogin`) par `storeTokens`/`storeUser`, supprimant les deux écritures `setJSON` brutes et rétablissant le stockage chiffré. Depuis le 14 juillet 2026, le champ `oauth_profile: calendar-events-v1` est contrôlé par le serveur et stocké dans le même blob que les jetons ; seul un grant portant ce profil peut fournir un fallback de `refresh_token`. Le `refresh_token` initial reste obtenu via `requestServerAuthCode`/l'échange de code.
...
- **Clé navigateur Picker — exception RÈGLE 1 actée (D8), restrictions alignées sur D32/D35** : le décideur autorise explicitement une **clé publique restreinte**. Referrers console EXACTS (syntaxe Google, variantes `/*`) : `https://tryarty.com/*`, `https://preview.tryarty.com/*`, `http://localhost:5173/*` — rien d'autre (`*.appfacade.pages.dev`, `capacitor://localhost`, `https://localhost` RETIRÉS : le Picker web n'y tourne pas). `www.tryarty.com` : **redirection 301 vers `tryarty.com`** (livrable PR-B1) — à défaut, ajouter `https://www.tryarty.com/*` ici ET `https://www.tryarty.com` dans `GIS_ALLOWED_ORIGINS`. Restriction API « Google Picker API » uniquement ; clé **distincte** de la clé Maps de `geo/reverse`. Modalités (D32) : l'exception est matérialisée **nominativement dans QUATRE endroits** à l'implémentation (PR-B1) — (i) `CLAUDE.md` **RÈGLE 1** (« exception nominative : `VITE_GOOGLE_PICKER_API_KEY`, clé Google navigateur non-secrète, referrer- et API-restreinte ») ; (ii) `CLAUDE.md` **RÈGLE 4** (le point « AUCUNE clé API dans le code JavaScript » de la checklist Play Store devient « aucune clé API hors exception nominative RÈGLE 1 ») ; (iii) le commentaire de `src/vite-env.d.ts` (« NEVER add VITE_*_API_KEY » : exception listée) ; (iv) `.env.example` + documentation de déploiement (variable documentée avec ses restrictions console obligatoires). Le nom reste **honnête** (`VITE_GOOGLE_PICKER_API_KEY`) — PAS de renommage destiné à contourner le scanner (D8) : c'est l'exception documentée qui couvre le grep d'audit, pas un nom qui l'esquive.
...
- **Stockage CHIFFRÉ (D22)** : le registre ne reste PAS en `setJSON` clair — les noms de fichiers sont des données personnelles et la `resourceKey` est un secret d'accès. Stockage sous `drive-connected-files-enc` (AES-256 au repos) avec **cache mémoire déchiffré** pour les lectures synchrones (pattern conversations/BUG 16 — PAS `secureSetJSON` sur une clé lue en synchrone, BUG 1) ; bootstrap async + event `*-storage-ready` (pattern BUG 43). Contenu : `id`, `name`, `mimeType`, `resourceKey?`, `grantedAt` — PAS de base64, PAS de contenu de fichier (BUG 11). **Purge explicite (D31)** : au `logout()` ET au changement de compte, **supprimer `drive-connected-files-enc` et remettre à zéro le cache Drive déchiffré en mémoire AVANT `setActiveSession()`/`clearActiveSession()`** — `clearActiveKeys()` ne réalise PAS cette purge (il n'efface que les clés API actives) ; c'est une étape dédiée, dans l'esprit de l'ordre BUG 6 et de la passe BUG 41. Toute écriture dispatch un `CustomEvent` **`drive-connected-files-updated`** (pattern BUG 54), écouté par l'écran Réglages et la carte contextuelle.
...
Tout scope Gmail (`gmail.*`, `mail.google.com`), `drive`, `drive.readonly`, `drive.metadata`, `drive.metadata.readonly`, `drive.activity*`, `drive.meet.readonly`, `drive.scripts`, `contacts`. Tests : parité TS (`gmailNoCasaPhase0.test.ts`, étendue : `drive.file` ABSENT du login) + test `.java` à créer (§4.2) + **contrôle runtime serveur** (§4.0 P0-a : rejet des jetons porteurs de scopes restreints).

### 5.4 Vérification Google

`drive.file` est non-sensible : **aucune vérification renforcée, aucun CASA**. La vérification de marque standard reste requise. `calendar.events` (sensible) doit figurer dans la demande de vérification OAuth. **Prérequis P0-b (D10)** : preuve documentée que les scopes restreints du `growth-orchestrator` vivent dans un projet OAuth entièrement séparé.
...
|---|---|---|---|---|---|
| `functions/api/drive/action.ts` (réactivé, opérations réduites : `list`, `read`, `download`, `create` — la « recherche » est `list`+`q`, l'« export » vit dans `read`/`download`) | **DEUX justificatifs (D13)** : identité/session Arty (header existant) + jeton Drive contextuel (`x-drive-token`) ; `tokeninfo` sur les deux : jeton Drive == **exactement `drive.file`** (D12), identité en allowlist exacte des scopes publics (D20), **tuple ordonné exact `(azp, aud)` par type de jeton** (allowlist figée par le gate §8.0 ; `aud` valide + `azp` étranger → 403), **`sub` obligatoire et identique** entre les deux jetons, expiration/claims en échec fermé (D25/D30/D37, §4.0 P0-a) | le token de l'utilisateur EST l'autorité — Google filtre le périmètre du scope ; le **registre** filtre en plus côté app (D4) ; IDs validés regex (BUG 32) ; terme `fullText` échappé ; `X-Goog-Drive-Resource-Keys` uniquement pour les ids du registre (D5) | pas de clé owner dépensée (API Drive gratuite) ; quotas/caps par appel conservés (bornes précises ci-dessous) | erreurs Google masquées (générique + `console.error`, pattern N-2/PR #307) | Origin whitelist stricte inchangée ; **listes `Access-Control-Allow-Headers` étendues à `x-drive-token`** (`:92` + `:127`, D20) |
...
**Bornes du proxy (D22, valeurs précisées par D27)** — valeurs par défaut, ajustables uniquement par mise à jour de ce CDC :

| Borne | Valeur |
|---|---|
| Corps de requête (hors `create`) | ≤ 64 Ko |
| **`create` : corps BRUT de la requête** (D33) | ≤ 1,5 Mo (contenu + métadonnées + surcoût d'encodage), vérifié AVANT parsing |
| `create` : contenu | ≤ 1 Mo ; nom ≤ 255 caractères (nom REJETÉ si contrôles/bidi présents) |
| `read`/`download` : taille de fichier acceptée | ≤ 15 Mo (au-delà : erreur claire ; `files.export` est déjà plafonné 10 Mo par Google) |
| Terme de recherche (`q`) | ≤ 256 caractères après échappement |
| Pagination | `pageSize` ≤ 50 ; ≤ 3 pages par appel d'outil (au-delà : tronqué + signalé au LLM) |
| `fileId`/`folderId` | regex `^[a-zA-Z0-9_-]+$`, longueur ≤ 128 (BUG 32) |
| `resourceKey` (D33) | **chaîne OPAQUE bornée** — longueur ≤ 128 ; REJET si contrôles (`\x00-\x1F`, `\x7F`), CR/LF, `/` ou `,` (délimiteurs du header `X-Goog-Drive-Resource-Keys`) ; **PAS de regex alphanumérique imposée** (format non documenté par Google — [contrat officiel](https://developers.google.com/workspace/drive/api/guides/resource-keys)) |
| Délais | `googleFetch` 20 s (pattern C13/PR #314) |
| **Allowlist MIME lecture** (chaînes canoniques, D33) | `application/vnd.google-apps.document`, `application/vnd.google-apps.spreadsheet`, `application/vnd.google-apps.presentation` (via `files.export` → `text/plain`/`text/csv`) ; `application/pdf`, `image/png`, `image/jpeg`, `image/webp`, `image/gif`, `text/plain`, `text/markdown`, `text/csv`, `application/vnd.openxmlformats-officedocument.wordprocessingml.document`, `application/vnd.openxmlformats-officedocument.spreadsheetml.sheet`, `application/vnd.openxmlformats-officedocument.presentationml.presentation` — tout autre type → erreur explicite « type non supporté » |
| **Allowlist MIME création** (chaînes canoniques, D33) | `text/plain`, `text/markdown`, `text/csv`, `application/pdf`, `application/vnd.google-apps.document` |

Toute valeur hors borne → 400 générique, jamais d'appel Google. **Chaque frontière est testée à limite−1 (passe), limite (passe), limite+1 (400) + un test de timeout** (D33).
...
- **Registre local** : métadonnées uniquement, **chiffré** (`drive-connected-files-enc` + cache mémoire, D22 — voir §4.4), purge au `logout()` ET au changement de compte, event BUG 54.
...
| ID | Exigence |
|---|---|
| DRV-P1-00a | Contrôle serveur à deux justificatifs : jeton Drive == exactement `drive.file` (D12), identité en allowlist exacte, tuples ordonnés `(azp, aud)` réels par type de jeton (gate §8.0, pas de produit cartésien — D30), `sub` obligatoire et identique, expiration/claims en échec fermé (D3/D13/D25) |
| DRV-P1-00d | PR-0 verte seule : scanners APK/AAB sémantiques inclus, quatre connecteurs (Drive/Gmail/Contacts/Sheets) en tombstones HTTP 410, tous contrôles verts ; PR-B0 retire atomiquement Drive SEUL des tombstones (D29) |
...
| ID | Exigence |
|---|---|
| GML-P1-08a | Lien direct résultats Gmail web desktop sous flag `VITE_GMAIL_SEARCH_DIRECT_LINK` **indépendant du profil Phase 0**, navigation « onglet réservé » sur TOUT le web (flag ON/OFF, desktop + mobile web) : copie terminée AVANT navigation, un seul onglet, jamais de `location.assign` automatique ; **trois états d'échec distincts (D36)** : `URIError`/re-validation → `needs_details` sans `window.open` ; copie OK + ouverture KO (popup, onglet fermé, navigation) → `copied_manual` ; copie refusée → `copy_error` (requête sélectionnable, jamais « copiée ») (D11/D15/D24/D34/D36) |
...
| DRV-P1-03 | Registre « Mes fichiers connectés » chiffré : métadonnées + `resourceKey` (D5/D22), event BUG 54, purge logout ET switch de compte AVANT `setActiveSession()` (D31), **filtrage réel** de `list`/`search`/`read` (D4), wording honnête + lien révocation Google |
...
| DRV-P1-08 | Cycle de vie Android complet : `play-services-auth` 21.6.0, renouvellement sans Picker ni `Prompt.CONSENT` (opt-out conservé, CTA si interaction requise — D19), métadonnées + `resourceKey` via `files.get` après `picked_file_ids` (D14) |
...
### 8.0 `GATE-TOKEN-PAIRS-01` (½ journée, pendant PR-B0, AVANT de figer `AUTHORIZED_CLIENT_PAIRS`) — D30

Sur de VRAIS jetons (pas de valeurs supposées) : obtenir les quatre types de jetons — **identité Web** (login redirect), **Drive Web** (GIS `initTokenClient`), **identité Android** (serverAuthCode échangé), **Drive Android** (`AuthorizationClient`) — appeler `tokeninfo` sur chacun et **consigner les tuples ordonnés `(azp, aud)` observés**. L'allowlist `AUTHORIZED_CLIENT_PAIRS` est figée à partir de ces valeurs consignées, un tuple par type de jeton — **jamais par produit cartésien** de listes.

**Anti-circularité (D37)** : les flux GIS (PR-B1) et `AuthorizationClient` (PR-B4) n'existent pas encore au moment de PR-B0 → un **harness temporaire Web/APK** est créé en DÉBUT de PR-B0 (page de test locale GIS + APK de test `AuthorizationClient` minimal, hors code de production, supprimés après le gate) uniquement pour émettre les quatre jetons et lire leur `tokeninfo`.

**Fermeture formelle (D37)** : le gate n'est CLOS que quand (a) le résultat est annexé à ce CDC (date, plateforme, valeurs expurgées des suffixes client) ET (b) la suite de tests contient **un test POSITIF par tuple réel** (les quatre acceptés) en plus des cas de rejet. Ce gate figure dans les critères d'acceptation (§10.9).

### 8.1 `SPIKE-DRIVE-FULLTEXT-01` (½ journée, APRÈS PR-B2 et AVANT PR-B3 — le spike a besoin de fichiers accordés via le Picker)
...
Sur APK réel (pas émulateur seul), avec `play-services-auth` **21.6.0** : `AuthorizationClient` + `AuthorizationRequest` (`drive.file` seul, `setOptOutIncludingGrantedScopes(true)`, `PICKER_OAUTH_TRIGGER`, `Prompt.CONSENT`). Critères : l'écran de consentement n'affiche QUE `drive.file` ; le Picker système s'affiche et la recherche fonctionne ; la sélection retourne `picked_file_ids` et **`files.get(fields=...resourceKey...)` complète les métadonnées** ; le couple identité+jeton Drive est accepté par le proxy (contrôle §4.0 P0-a : scope exact, couples `aud`/`azp`, `sub` obligatoire identique — D25) ; **renouvellement après expiration/kill de l'app SANS réouverture du Picker ni `Prompt.CONSENT`, opt-out conservé** — si Google exige une interaction, le CTA « Reconnecter mes fichiers » s'affiche (D19) ; comportement multi-comptes sain (`SELECT_ACCOUNT` + rejet `sub` divergent). FAIL → volet B livré web-first ; Android dans une PR dédiée ultérieure. Résultat annexé à ce CDC (date, appareil, captures).

### 8.3 Matrice manuelle lien direct Gmail (½ journée)

Chrome/Firefox/Safari desktop × {1 compte, multi-comptes, non connecté} × {flag ON, flag OFF} + mobile web (Chrome Android, Safari iOS — racine Gmail via l'ouvreur commun) : le lien ouvre les bons résultats ou échoue vers l'état ATTENDU par cas (D36) — popup bloquée / onglet fermé / navigation impossible (copie OK) → `copied_manual` ; copie refusée → `copy_error` (requête sélectionnable, aucune mention « copiée ») ; `URIError` → `needs_details`, aucun `window.open`. Vérifier l'ABSENCE de double navigation dans tous les cas. Annexer la matrice à la PR.
...
- Navigation « onglet réservé » (D15/D24/D36) : URL validée/construite AVANT réservation ; onglet ouvert dans le geste utilisateur, navigué APRÈS résolution de la copie, un seul onglet, `opener` nul ; **les trois états testés distinctement** : `URIError`/re-validation KO → `needs_details` sans `window.open` ; copie OK + {popup bloquée | onglet fermé | navigation qui lève} → `copied_manual` (« Requête copiée » + bouton Gmail) ; copie refusée → `copy_error` (requête sélectionnable + bouton Gmail, **sans** mention « copiée ») ; jamais de `location.assign` automatique ; mêmes tests pour flag OFF et mobile web (racine Gmail).
...
- Contrôle serveur à deux justificatifs (D25/D37) : **tests POSITIFS des quatre tuples réels** (identité Web, Drive Web, identité Android, Drive Android — chacun accepté avec les valeurs consignées par §8.0) ; puis rejets : jeton Drive avec scope surnuméraire → 403 ; `drive.file` manquant → 403 ; jeton d'identité hors allowlist exacte des scopes publics → 403 ; tuple `(azp, aud)` hors allowlist → 403 ; **`aud` valide + `azp` étranger → 403 (test canonique)** ; **`sub` divergent entre les deux jetons → 403 ; `sub` ABSENT sur l'un des jetons → 403 (échec fermé)** ; jeton expiré → 403 (mocks tokeninfo, pattern `audValidation.test.ts`).
- Ouvreur web — couvert par le point « onglet réservé » ci-dessus (trois états D36) ; test supplémentaire : l'état `copy_error` ne contient PAS le texte « copiée » (anti-régression sur le mélange d'états).
- `origin_mismatch` (D32) : `location.origin` hors `GIS_ALLOWED_ORIGINS` (preview dynamique) → bouton « Connecter » absent + explication ; origine autorisée exacte (schéma+port) → bouton présent.
- Tombstones (D29) : les quatre connecteurs répondent 410 après PR-0 ; après PR-B0, Drive répond ET Gmail/Contacts/Sheets répondent toujours 410 (test de non-régression atomique).
- Frontières (D33) : pour CHAQUE borne du tableau §6.1 — limite−1 (passe), limite (passe), limite+1 (400) ; + un test de timeout `googleFetch` ; `resourceKey` : rejet de `\n`, `\r`, `/`, `,`, contrôles ; acceptation d'une clé opaque non alphanumérique valide.
...
1. Sur web desktop, flag ON : « retrouve le mail de Paul sur le devis de juin » → carte avec requête → un clic → **la copie est terminée avant la navigation**, Gmail ouvert **sur les résultats**, **un seul onglet** ouvert. Mêmes garanties flag OFF et sur mobile web (racine Gmail). Tout échec aboutit à l'état DISTINCT attendu (D36) : copie OK + ouverture KO → `copied_manual` (« Requête copiée » + bouton Gmail) ; copie refusée → `copy_error` (requête sélectionnable, jamais annoncée copiée) ; `URIError` → `needs_details` sans aucun `window.open` — jamais un silence, jamais un mélange d'états.
...
5. Un jeton portant un ancien scope large (`drive`, `gmail.readonly`) est **refusé** par le proxy Drive ; un jeton Drive portant un scope EN PLUS de `drive.file` est **refusé** ; un jeton d'un compte différent du compte de session (`sub` divergent ou absent) est **refusé** ; un jeton avec **`aud` valide mais `azp` étranger** est **refusé** (D25) — chaque cas avec message de reconnexion actionnable.
...
5-ter. Le préflight CORS passe avec `x-drive-token` (les deux listes du middleware à jour) ; le registre est illisible en clair dans localStorage (noms + `resourceKey` chiffrés) et disparaît au logout comme au changement de compte — purge exécutée AVANT `setActiveSession()` (D31).
5-quater. Après PR-0 : les quatre connecteurs Google répondent **410** et TOUS les contrôles (scanners sémantiques, 3 inspections APK/AAB, `no-casa:check`, suite de tests) sont **verts**. Après PR-B0 : Drive seul est réactivé, Gmail/Contacts/Sheets répondent toujours 410 (D29). Chaque borne du proxy a ses tests limite−1/limite/limite+1 + timeout (D33).
...
9. Les gates **§8.0** (fermé formellement : quatre tuples réels annexés + un test positif par tuple dans la suite — D37), §8.1 et §8.2 sont exécutés et annexés ; la preuve P0-b (growth-orchestrator) est annexée.
...
|---|---|---|
| **PR-0** | **Fondation construite from scratch (I1/D29)** : scanner sémantique `check-public-google-access.mjs` (scopes publics exacts, URL Gmail légitime non confondue avec un scope), inspections du manifest mergé + APK debug et inspection AAB dans le workflow release, renommages internes `PUBLIC_GOOGLE_*`, migration de `App.tsx` vers `storeTokens`/`storeUser`, exclusion Sheets explicite, **quatre connecteurs en tombstones HTTP 410**, variable de rollback serveur documentée et suite entièrement verte (`no-casa:check` inclus). Le profil atomique `calendar-events-v1`, ajouté le 14 juillet, devient un prérequis déjà satisfait pour PR-B0. Prérequis de TOUTES les PRs suivantes | 2-2,5 j (décideur + Claude) |
| PR-A1 | Compilateur discriminé + triggers élargis/anti-collision/faux positifs + paire adverse colis/mail (D17/D24) + tests d'opérateurs compilés (`from:Paul`, D27) + system prompt/brief/demo/hygiène + réécriture sémantique des tests unitaires restants (D18 — les scanners sont en PR-0) | 1,5-2 j |
| PR-A2 | Lien direct sous flag indépendant + ouvreur « onglet réservé » remplaçant `openGmailHome()` sur TOUT le web (flag ON/OFF, desktop + mobile web, D27) + échecs actionnables « Requête copiée » + bouton Gmail manuel (D34) + re-validation au clic (D16) + validateur authuser + matrice étendue | 1,5 j |
| **PR-B0** | **Harness temporaire Web/APK en DÉBUT de PR (D37, anti-circularité) → `GATE-TOKEN-PAIRS-01` §8.0 fermé formellement (4 tuples réels + tests positifs) ; retrait ATOMIQUE de Drive seul des tombstones (D29) + protections D3/D12/D13/D25/D30 : contrôle serveur à deux justificatifs (`sub` obligatoire, tuples ordonnés exacts, allowlist scopes, expiration fail-closed) + `x-drive-token` au CORS (les 2 listes) + bornes précises du proxy avec tests de frontières (D33) + révocation/migration anciens grants + preuve growth-orchestrator** | 3 j + ops |
| PR-B1 | Autorisation contextuelle web (GIS `include_granted_scopes: false`, `GIS_ALLOWED_ORIGINS` exactes + test `origin_mismatch` D32, CSP accounts.google.com) + **livrables DNS/infra : création `preview.tryarty.com` + redirection 301 `www.tryarty.com` → `tryarty.com` (D35)** + referrers clé Picker alignés D35 + état/CTA « Reconnecter mes fichiers » (D19) + amendements nominatifs RÈGLES 1/4 + `vite-env.d.ts` + `.env.example` + doc déploiement (D32) + tests parité (TS + `.java`) | 2 j |
| PR-B2 | Picker web + registre **chiffré** filtrant (D22) + purge switch de compte AVANT `setActiveSession()` (D31) + resourceKeys opaques (D33) + CSP Picker | 2-2,5 j |
| — | `SPIKE-DRIVE-FULLTEXT-01` (§8.1) — après B2, conditionne la promesse UI de B3 | ½ j |
...
**Ordre (verdict 4)** : **PR-0 → A1 → A2 → B0 → B1 → B2 → spike fullText → B3 → B4**. Chaque PR passe `tsc`, la suite complète, et l'audit RÈGLE 6 quand un endpoint est touché.
...
Aucune question ouverte : les décisions D1-D37 (§0) couvrent les questions de la v1.1 et les blocages des verdicts 2 à 6 (D29/D31/D33 + invariant OAuth public : validés au verdict 6). Contrat de livraison : **chaque PR passe seule** (PR-0 verte avec les quatre connecteurs en 410 ; PR-B0 réactive Drive atomiquement) et **chaque garantie a un test observable** (frontières limite±1, `origin_mismatch`, tombstones, tuples réels §8.0 avec tests positifs, trois états d'échec Gmail distincts). L'implémentation est engagée selon l'ordre PR-0 → A1 → A2 → B0 → B1 → B2 → spike fullText → B3 → B4.
