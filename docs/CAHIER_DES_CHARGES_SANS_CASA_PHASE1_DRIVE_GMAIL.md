...
**Version :** 1.4.1 — quatrième verdict du décideur intégré (13 juillet 2026)<br>
**Date :** 13 juillet 2026<br>
**Statut :** GO sur l'architecture hors CASA ; **implémentation EN PAUSE** jusqu'au GO sur cette v1.4.1. Aucun code n'est écrit d'ici là.<br>
...
| `create` : contenu | ≤ 1 Mo ; nom ≤ 255 caractères (contrôles + bidi strippés) |
...
| `resourceKey` | regex `^[a-zA-Z0-9_-]+$`, longueur ≤ 128, validée AVANT interpolation dans le header |
| Délais | `googleFetch` 20 s (pattern C13/PR #314) |
| **Allowlist MIME** (lecture) | Google natifs : Docs/Sheets/Slides (via `files.export` → `text/plain`/`text/csv`) ; binaires : `application/pdf`, `image/png`, `image/jpeg`, `image/webp`, `image/gif`, `text/plain`, `text/markdown`, `text/csv`, docx/xlsx/pptx (OOXML) — tout autre type → erreur explicite « type non supporté » |
| **Allowlist MIME** (création) | `text/plain`, `text/markdown`, `text/csv`, `application/pdf`, Google Docs |

Toute valeur hors borne → 400 générique, jamais d'appel Google.
...
| ID | Exigence |
|---|---|
| DRV-P1-00a | Contrôle serveur à deux justificatifs : jeton Drive == exactement `drive.file` (D12), identité en allowlist exacte, `aud`+`azp` validés séparément contre les couples Web/Android autorisés, `sub` obligatoire et identique, expiration/claims en échec fermé (D3/D13/D25) |
...
- Ouvreur web : `encodeURIComponent` qui lève `URIError` (surrogate isolé) → `needs_details`, aucun onglet ; échec d'assignation `tab.location.href` → onglet fermé + message.
...
|---|---|---|
| **PR-0** | **Stabilisation de la migration locale (D26)** : push des 31 fichiers modifiés + 5 nouveaux, scanner `check-public-google-access.mjs` rendu SÉMANTIQUE (accepte `drive.file` et l'URL Gmail légitime), renommages `PUBLIC_GOOGLE_*` complets, réparation du test qui importe le module supprimé `gmailNoCasaPhase0`, fix `App.tsx:957` (`storeTokens`/`storeUser` + `oauth_profile`), **suite entièrement verte** (`no-casa:check` inclus). Drive/Gmail/Contacts/Sheets restent DÉSACTIVÉS. Prérequis de TOUTES les PRs suivantes | 1,5-2 j (décideur + Claude) |
| PR-A1 | Compilateur discriminé + triggers élargis/anti-collision/faux positifs + paire adverse colis/mail (D17/D24) + tests d'opérateurs compilés (`from:Paul`, D27) + system prompt/brief/demo/hygiène + réécriture sémantique des 3 inspections APK/AAB et tests unitaires restants (D18/D21) | 2 j |
| PR-A2 | Lien direct sous flag indépendant + ouvreur « onglet réservé » remplaçant `openGmailHome()` sur TOUT le web (flag ON/OFF, desktop + mobile web, D27) + gestion échecs navigation/`URIError` + re-validation au clic (D16) + validateur authuser + matrice | 1,5 j |
| **PR-B0** | **Réactivation Drive UNIQUEMENT (D26) + protections D3/D12/D13/D25 : contrôle serveur à deux justificatifs (`sub` obligatoire, couples `aud`/`azp` autorisés, allowlist scopes, expiration fail-closed) + `x-drive-token` au CORS (les 2 listes) + bornes précises du proxy (tableau §6.1) + révocation/migration anciens grants + preuve growth-orchestrator** | 2-2,5 j + ops |
| PR-B1 | Autorisation contextuelle web (GIS `include_granted_scopes: false`, alias preview fixe D28, CSP accounts.google.com) + état/CTA « Reconnecter mes fichiers » (D19) + tests parité (TS + `.java`) | 1,5 j |
...
Aucune question ouverte : les décisions D1-D28 (§0) couvrent les questions de la v1.1 et les blocages des deuxième, troisième et quatrième verdicts (D23 tranché par D28 : alias preview fixe). Une seule dépendance externe : **PR-0 (stabilisation de la migration locale, D26)** ouvre la séquence. L'implémentation reste **en pause** jusqu'au GO explicite du décideur sur cette v1.4.1.
