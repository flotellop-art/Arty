...
**Statut :** architecture hors CASA validée ; D29/D31/D33 et l'invariant OAuth public validés par le décideur ; trois retouches mécaniques intégrées (D35-D37). **Implémentation EN PAUSE** jusqu'au GO sur cette v1.4.3 — annoncé comme dernier tour avant GO sans nouvelle revue d'architecture.<br>
...
| I2 | **Coupure immédiate RATIFIÉE** (question posée explicitement, réponse : « Coupure immédiate ») : les tombstones 410 sont **actifs PAR DÉFAUT** au déploiement, **avant toute auth** (uniformes, aucun aller-retour Google). `LEGACY_GOOGLE_CONNECTORS_ENABLED='true'` (env Cloudflare) = variable d'échappement de **rollback d'urgence** (poser la var puis « Retry deployment », ~2 min, sans revert de code — sur Pages une variable ne s'applique qu'au déploiement suivant). ⚠️ Le client prod (flag Phase 0 OFF) garde scopes et outils tant que le **flip client** (`VITE_GMAIL_NO_CASA_PHASE0=true` sur Cloudflare + build Android flag ON) n'est pas déployé — bascule coordonnée à la main du décideur. |
...
|---|---|---|
| **PR-0** | **Stabilisation de la migration locale (D26/D29)** : push des 31 fichiers modifiés + 5 nouveaux, scanner `check-public-google-access.mjs` rendu SÉMANTIQUE (accepte `drive.file` et l'URL Gmail légitime), **les 3 inspections APK/AAB sémantiques incluses** (introduites par la migration — D29), renommages `PUBLIC_GOOGLE_*` complets, réparation du test qui importe le module supprimé `gmailNoCasaPhase0`, fix `App.tsx:957` (`storeTokens`/`storeUser` + `oauth_profile`), **quatre connecteurs en tombstones HTTP 410**, **suite entièrement verte** (`no-casa:check` inclus). Prérequis de TOUTES les PRs suivantes | 2-2,5 j (décideur + Claude) |
