...
2. [x] **Merger la PR #338** (squash, comme #336/#337) — fusionnée le 13 juillet à 21 h 39 CEST, commit `c801442`.
3. [x] **Vérifier en prod** (~5 min après) : contrôle final refait sur le commit `f601b50` (déploiement Cloudflare `11dbf591`, vert). Le bundle canonique `index-BGLQGFmS.js` contient le parcours « Préparer la recherche Gmail » et la promesse « Arty ne lit pas ta boîte Gmail » ; Gmail, Drive, Contacts et Sheets répondent tous `410 {"error":"Gone"}`. Calendar atteint encore son handler fail-closed (`404`, jamais le tombstone 410). L'APK inspecté conserve exactement les permissions caméra, micro et géolocalisation prévues dans l'allowlist stricte.
4. [x] **Android** : PR #340 fusionnée (`f601b50`) ; build Firebase `29282379633` exécuté avec `VITE_GMAIL_NO_CASA_PHASE0=true` pour le WebView ET `ARTY_GMAIL_NO_CASA_PHASE0=true` pour le natif. La release signée **1.0.80 (81)** a été distribuée avec succès au groupe `beta-testers`. Contrôles avant envoi : `BuildConfig.GMAIL_NO_CASA_PHASE0=true`, certificat officiel, 13 permissions exactes, 1103 tests verts et smoke navigateur des deux demandes « Quel est le dernier mail reçu ? » / « Résume mes mails non lus » sans 410 ni affirmation inventée. La build automatique 1.0.79 (80), identifiée comme legacy, est donc supersédée.
