...
### Campagne de reprise et réserves

La campagne complète locale avec couverture et deux workers a terminé en 1 105 secondes : 385 fichiers réussis, deux en échec ; 5 666 tests réussis, quatre échecs et un ignoré. Deux échecs sont des expirations explicites du délai Vitest de 5 secondes dans les tests existants de vingt réservations D1 simultanées. Les deux autres concernent le client TikTok transformé avant son raccord au reçu de financement, alors que les nouveaux tests ont été lus après modification : cette campagne ne représente donc pas un instantané uniforme du candidat final. Ne pas la déclarer entièrement verte.

Après fin de cette campagne, un seul passage ciblé sur les cinq fichiers concernés et leurs raccords (budget D1, client TikTok, conversation TikTok, proxy Gemini, refus de financement) a réussi : **180 tests**, couverture active, deux workers, délai de test CLI de 20 secondes, toutes les assertions financières conservées. Cela confirme notamment une seule réservation parmi vingt et les totaux D1. Aucun délai du produit ni fichier de test du budget n'a été changé. Les 97 tests précédents du raccord client avaient également réussi avec le délai habituel.

Le processus ciblé retourne néanmoins un échec des seuils de couverture globaux, puisque cinq fichiers ne couvrent pas toute l'application : ce n'est pas une validation de la couverture globale. Journaux locaux : `tiktok-release-tests.log`, `tiktok-release-targeted.log`, `tiktok-release-build.log`. Les contrôles de publication et la recette fournisseur réelle restent requis ; la livraison est proposée en brouillon.
