...
- [ ] **Pas de rate limit sur `/api/auth/token`** — TOUJOURS présent, requalifié **LOW** : exploit quasi nul (exige le `client_secret` owner + codes OAuth single-use / TTL court). Durcissement anti-bruteforce de `refresh_token` volés seulement. **Déféré volontairement en C13 (PR #314, 4 juillet 2026)** : un rate-limit par IP sur `auth/refresh` (appelé à chaque reprise de session) risque de bloquer des users légitimes derrière une IP partagée — à ne faire qu'en PR dédiée avec un design qui évite ce piège.

**Nouveaux findings résiduels (audit 7 juin)** :
- [x] **N-1 (MED) — validation `aud`** : CLOS le 4 juillet 2026 (C1/F-9, PR #309). Étape 1 (commit `cb8f222`) : `verifyGoogleUser(request, expectedAud?)` valide l'audience via `tokeninfo`, passé par tous les endpoints qui dépensent la clé IA owner (proxys via `resolveProxyIdentity`, tts, image-gen, memory-extract, whisper, voxtral, checkout, trial/init, subscription/status). Étape 2 (PR #309) : les chemins `checkAllowedUserPeek`/`checkAllowedUser` passent désormais `env.GOOGLE_CLIENT_ID` → couvre `search/web.ts`, `fetch/url.ts` (clés Linkup/Brave owner), `ai/quota/*`, météo, géo. Sémantique FAIL-SAFE délibérée (BUG 21/51) : seule une audience étrangère EXPLICITE est rejetée ; `aud`/`azp` absents (tokens natifs serverAuthCode) ou tokeninfo KO ne verrouillent PAS. 9 tests unitaires (`audValidation.test.ts`, mock tokeninfo). Reste à tracer : test terrain APK réel (login natif → recherche web) — risque faible grâce au fail-safe.
...
- [x] **MED — N-1 `aud`** : CLOS le 4 juillet 2026 (C1/F-9, PR #309) — voir ci-dessus.
...
- [x] **LOW — `computer/relay` forwarde une action arbitraire** — FAIT le 4 juillet 2026 (C13, PR #314) : allowlist d'actions type `ComputerAction` (screenshot/open_app/click/type/scroll/key) au relay + tests (`relayAllowlist.test.ts`). Le serveur local était déjà durci (`125dcd1`).
- [x] **LOW — permissions Android sur-déclarées** — FAIT les 4-5 juillet 2026 (C7/F-28, PR #316 + commit `d1d4968`) : `READ_MEDIA_AUDIO`/`READ_MEDIA_VIDEO` retirées. ⚠️ `MODIFY_AUDIO_SETTINGS` RÉTABLIE après test APK réel KO : getUserMedia en WebView Capacitor (pipeline WebRTC/AudioManager) exige cette permission — sans elle le micro est refusé MÊME avec `RECORD_AUDIO` accordée (NotReadableError). Ne PAS la re-retirer ; commentaire d'usage dans le manifest. **Timeouts serveur→Google** : FAIT le 4 juillet (C13, PR #314) — helper `googleFetch` (AbortSignal.timeout 20 s) appliqué aux 43 fetch googleapis de gmail/drive/calendar/contacts/sheets. FAIT le 3 juillet (PR #307) : `gemini-proxy` valide le format de `model` ; `drive` encode `previousParents`.
...
**Corrigé les 4-5 juillet 2026 (chantiers Cowork C1-C13 du CDC `docs/audits/cdc-cowork-2026-07-restant.md`, PRs #309-#316 — vérifié par contre-audit Fable le 5 juillet : code relu, typecheck vert, CI verte sur HEAD, prod sondée)** :
- [x] **C1 (F-9) — validation `aud` chemins peek** (PR #309) : voir N-1 ci-dessus (clos).
- [x] **C2 (F-10) — Turnstile** : front câblé (PR #313 : `TurnstileWidget.tsx` + `EmailTrialFlow`, anti-lockout fail-open front, 3 tests, CSP `challenges.cloudflare.com` en script-src/frame-src) ; **ops VÉRIFIÉ FONCTIONNELLEMENT le 5 juillet** : sitekey présente dans le bundle prod tryarty.com ET sonde `request-otp` avec token bidon → `403 captcha_failed` (secret posée, vérif active, aucun email parti). **Volet code fail-closed FAIT le 5 juillet 2026 (décision Florent)** : `turnstileMisconfiguredInProd()` (`emailTrial.ts`) + gate 503 en tête de `request-otp` — en prod, secret manquante = refus explicite au lieu de fail-open silencieux. Détection prod par HOSTNAME (`PRODUCTION_HOSTS`, miroir des hosts HTTPS de `ALLOWED_ORIGINS`) et PAS par `CF_PAGES_BRANCH` (présence au runtime des Pages Functions non documentée → gate qui ne s'armerait jamais). Anti-dérive : test de parité CI `PRODUCTION_HOSTS` ⇄ `ALLOWED_ORIGINS` (pattern F-1) — tout nouveau domaine prod ajouté au middleware DOIT l'être aussi dans `PRODUCTION_HOSTS`, sinon CI rouge. Tout NOUVEL appelant de `verifyTurnstile` DOIT appliquer le gate en amont (doc dans le docblock). Relecture 2 agents (Opus sécu + Sonnet régressions) : GO ; i18n du 503 vérifié (fr/en `email_trial_unavailable`), natif couvert (apiBase → tryarty.com). RÉSIDUEL OPS VÉRIFIÉ + TRAITÉ le 5 juillet 2026 : le Preview PARTAGE bien `arty-db` (une seule D1 dans le compte) et avait Resend + EMAIL_FROM + EMAIL_TRIAL_SECRET SANS `TURNSTILE_SECRET_KEY`, previews publics par défaut → trou d'email-bombing sur tables prod, non couvert par le gate (qui ne vise que les hosts prod). CORRIGÉ (option 1) : les 3 vars email-trial RETIRÉES de l'environnement Preview → `request-otp` renvoie 503 avant tout envoi. ⚠️ N'affecte que les NOUVEAUX déploiements preview ; les previews déjà buildés gardent les anciennes vars → pour fermer les previews existants, activer « Restreindre les aperçus » (Cloudflare Access, option 2 non retenue) ou laisser expirer/supprimer les déploiements. Note hygiène : la plupart des vars Cloudflare sont en type « Texte brut » (visibles au dashboard) plutôt que « Secret » chiffré — RÈGLE 1 OK (aucun secret en `VITE_`), mais passer les clés en type Secret serait plus propre.
- [x] **C4 (F-27) — TODO Lemon Squeezy** re-daté explicitement (commit `1426275`) : fallback legacy `ALLOWED_EMAILS` à retirer après lancement commercial (aucun abonné à ce jour) ; `parseAllowedEmails` conservée (réutilisée + testée).
- [x] **C5 (F-11) — PKCE S256 sur OAuth Google web** (PR #310) : `buildOAuthUrl()` async (verifier 43 chars RFC 7636, sessionStorage), consommation SINGLE-USE dans `exchangeCode` (un seul point, discipline BUG 53), `auth/token.ts` forwarde `code_verifier` ; flow natif serverAuthCode intact (test `=== undefined`, BUG 2). Reste à tracer : test terrain login web desktop + PWA.
- [x] **C7 (F-28) — permissions Android** (PR #316 + `d1d4968`) : voir LOW ci-dessus — 2 retirées, `MODIFY_AUDIO_SETTINGS` rétablie après test APK KO (micro WebView).
- [x] **C8 (F-5) — tests d'intégration D1 réel** (PR #311) : harnais Miniflare + `schema.sql` de prod (`d1Harness.ts`), 4 zones couvertes : `consumeCapAtomic` sous concurrence, OTP (single-use DELETE…RETURNING, 5 tentatives max, rate-limits fail-closed), wallet (idempotence webhook, reserve/settle/void, jamais négatif), `resolveUserPlan` (matrice plans). Tourne en CI (`npm test`).
- [x] **C9 (F-20) — helper commun clients IA** (PR #312) : `src/services/aiHttp.ts` (`buildAiHeaders` : garde BUG 25 'server-provided' + `x-google-token` frais BUG 23 + repli trial-token ; `fetchWithTimeout` composable signal externe). 4 clients migrés ; parsing SSE volontairement PAS unifié (BUG 52).
- [x] **C13 — durcissements LOW** (PR #314) : timeouts Google 20 s (43 fetch), allowlist relay, clamp trial backend. Rate-limit auth déféré avec justification (voir LOW `/api/auth/token` ci-dessus) ; eslint non adopté.
- ℹ️ **Restent OUVERTS au CDC** : C2-code (⛔ décision Florent), C3 (go-live Creem), C6 (CryptoKey non-extractible — le plus gros), C10 (bundle 696 kB), C11 (god files), C12 (⛔ login local, Option A recommandée), C14 (quota transcription en secondes — si abus).
...
- ℹ️ **Différés (justification au CDC)** — MAJ 5 juillet : F-9, F-10, F-11, F-20, F-27, F-28 FAITS les 4-5 juillet (voir bloc ci-dessus). Restent : F-33 Creem go-live, F-34 chantier `CryptoKey` non-extractible — **priorité rehaussée** : le blob critique protégé par la passphrase publique est le `refresh_token` Google, pas seulement les conversations.
...
**MAJ 18 juillet 2026 (CDC veille modèles, C8)** — Small n'est plus jamais servi aux non-payants. **Free** : `allowed_families = ['claude-haiku']` — aucun accès Mistral, ni UI ni serveur. **Trial** : côté CLIENT, vu comme `free` (`normalizePlan` ne renvoie jamais `trial`) → chat Auto = Haiku uniquement en pratique ; côté SERVEUR, `TRIAL_ALLOWED_MODELS` autoriserait 4 familles dont `mistral-medium` (jamais Small, retiré en mai 2026) avec swap proxy vers Medium — branches aujourd'hui inatteignables via l'UI (décision D-C : statu quo jusqu'à la vigie C2). **Payants** : Small réservé aux messages triviaux, Medium 3.5 défaut.
...
1. Si `conversations-enc` existe mais est indéchiffrable avec la clé courante (passphrase changée, migration partielle, désync multi-comptes), `bootstrapConversationStorage()` laissait `cacheReady=false` pour TOUTE la session — « will retry next boot » mais le boot suivant échouait pareil → app définitivement inutilisable.
2. `createConversation()` n'avait PAS le garde `isCacheReady()` (contrairement à `sendMessage`, garde H5) → `saveConversation` no-op silencieux → navigation vers l'id d'une conversation qui n'existe nulle part.
3. `ChatRoute` : `if (id && (!activeConversation || activeConversation.id !== id)) return null` — `id` est TOUJOURS défini sur `/chat/:id`, donc ce `null` s'affichait pour toujours et la branche « Conversation introuvable » était du code mort. Sur mobile (sidebar cachée) → écran 100% vide.
**Règle** :
- Un échec de déchiffrement ne doit JAMAIS laisser l'app inutilisable pour toute la session. Pattern : QUARANTAINE (déplacer le blob vers `conversations-enc-locked`/`-locked-2`, jamais le supprimer) + cache vide utilisable + retentative de déchiffrement/fusion à chaque boot (`recoverLockedBlobs()`). Réversible dans les deux sens.
