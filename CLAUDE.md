...
Dernier audit : **14 juin 2026** (`/audit-secu` post-merge money-path crédits — agents Opus + vérif terrain manuelle). Précédent : 7 juin 2026 ; 4 mai 2026 (PR #127 + #128).

> **MAJ 14 juin 2026** — Audit déclenché après le merge en prod du money-path crédits (PR #238). **Aucun CRIT.** Concurrence wallet + idempotence top-up Creem = **solides** (`DB.batch` atomique + D1 mono-thread, vérifiés 2× indépendamment + doc Cloudflare). Findings actifs (non corrigés — décision « documenter d'abord ») :
> - **HIGH H-1 (N-1 requalifié)** — `verifyGoogleUser` (`checkAllowedUser.ts:15-34`) sans contrôle `aud`. Un token d'une AUTRE app Google passe le gate universel → abus des clés IA serveur / wallet / `memory` / `account/delete` sous l'identité d'une victime. PAS CRIT (gmail/drive re-forwardent le token → qui le possède tape Google en direct, Arty n'ajoute pas d'exfil) ; PAS MED (account/delete + memory agissent sous identité tierce). Fix = router le gate via `verifyTokenViaTokeninfo` (déjà écrit `:47-74`) ; **tester web ET natif** (`aud` des tokens `requestServerAuthCode`).
> - **HIGH H-2** — réserve wallet incomplète : `estimateReserveMicro` (`creditPricing.ts:111-128`) ne couvre que output + input **texte** ; `estimateInputTokens` (`walletBilling.ts:69`) ignore les blobs base64. Le settle facture `cache_creation` ($18.75/Mtok Opus) + input image → solde négatif ~$6-13/appel à la charge du owner, borné à ~1 appel/recharge mais déclenchable naturellement. Le commentaire `creditPricing.ts:108` (« fuite colmatée », fix 14 juin) ne couvre QUE l'input texte. Fix = réserver `cache_creation` pessimiste (si `cache_control` présent) + estimer les base64.
> - **MED M-1** — `consumePremiumPack` (`checkPremiumCap.ts:115-158`) PAS atomique (3 statements hors transaction) ; commentaire `:230` « déjà atomique » **faux**. Sur-débit € non exploitable aujourd'hui (garde `WHERE messages_used < messages_total` sérialisée) mais 0 marge. Fix = `UPDATE … RETURNING` atomique + corriger le commentaire.
> - **MED M-2** — webhook Lemon Squeezy sans idempotence (`lemonsqueezy.ts:200`, `INSERT OR REPLACE … messages_used=0`) → un re-delivery `order_created` recharge un pack consommé à 100. Fix = `ON CONFLICT DO NOTHING` / table d'idempotence (comme Creem).
> - **MED M-3** — double-débit refund (`creem.ts:167-183` + `wallet.ts:413`) : 2 events refund/dispute d'`event_id` ≠ sur le même `order_id` débitent 2× (index `settle_once` non collisionné car `ref_id=event_id`). Fix = dédup du débit par `order_id`.
> - **MED M-4** — chargeback → solde négatif + pas d'alerte (`wallet.ts:423`, `:463` `reconcileWallet`). Fix = alerte `WHERE balance_micro < 0`.
> - **MED M-5** — robustesse signature Creem (`creem.ts:74-80`) : casse hex + format header (`sha256=`/virgules) non validés contre un payload réel → risque de **rejeter 100 % des webhooks payés**. Valider AVANT go-live (perte revenus, pas vol).
> - **MED M-6/7/8** — injections path URLs Google sans validation ID : `contacts/action.ts:95` (`resourceName`), `wordpress/action.ts:55` (`postId`), `drive/action.ts:259` (`previousParents` non encodé) ; `fetch/url.ts:7` autorise `http:` (→ https-only) ; scope `drive` complet (`googleAuth.ts:19`) au lieu de `drive.file` (risque rejet vérif OAuth).
> - **LOW** — `cachedKey`/salt crypto non nettoyés au logout/switch ; fuites de message d'erreur (gmail attachment `:235`, contacts `:50`, memory `:103`) ; fallback `customer.email` non vérifié (don, pas vol) ; refund partiel sur-débite ; `refund.created` avant `checkout.completed` perdu ; `tool_use` non compté dans la réserve.
> - **Non re-audité ce pass** (agent frontend rate-limité) : intent-filters deeplink, iOS Info.plist, SW conditionnel. XSS markdown (`rehypeSanitize` dernier) / RÈGLE 1 / `sourcemap:false` / `allowBackup="false"` vérifiés OK à la main.
