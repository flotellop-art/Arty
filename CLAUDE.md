...
- [x] **C2 (F-10) — Turnstile** : front câblé (PR #313 : `TurnstileWidget.tsx` + `EmailTrialFlow`, anti-lockout fail-open front, 3 tests, CSP `challenges.cloudflare.com` en script-src/frame-src) ; **ops VÉRIFIÉ FONCTIONNELLEMENT le 5 juillet** : sitekey présente dans le bundle prod tryarty.com ET sonde `request-otp` avec token bidon → `403 captcha_failed` (secret posée, vérif active, aucun email parti). ⚠️ RÉSIDUEL ACCEPTÉ À STATUER : le volet code fail-closed (503 si prod sans `TURNSTILE_SECRET_KEY`) n'a PAS été codé — si la secret est un jour retirée par erreur, le flux redevient fail-open SILENCIEUSEMENT (`verifyTurnstile` retourne true sans clé).
