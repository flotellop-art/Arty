...
- [ ] **N-1 (MED) — `checkAllowedUser()` ne valide pas `aud` sur les 4 proxys IA principaux** — REQUALIFIÉ le 3 juillet : le helper `verifyGoogleUser(request, expectedAud?)` supporte `expectedAud` depuis le 14 juin et les endpoints périphériques (image-gen, tts, whisper, voxtral, memory-extract) l'utilisent déjà (`verifyGoogleUser(request, env.GOOGLE_CLIENT_ID)`). Mais `checkAllowedUser()` lui-même (`_lib/checkAllowedUser.ts:266-271`) n'a **aucun paramètre `expectedAud`** et appelle `verifyGoogleUser(request)` sans audience → les 4 proxys chat (`ai/proxy.ts:63`, `gemini-proxy.ts:56`, `mistral-proxy.ts:56`, `openai-proxy.ts:56`, tous `await checkAllowedUser(request, env)`) restent sans validation d'audience. C'est le chemin le plus coûteux (clé serveur owner, chat premium), donc le N-1 est fermé sur le mauvais périmètre. Impact : un token Google d'audience étrangère passe le gate → abus borné de la clé owner (PAS de vol de données). Fix : ajouter `expectedAud` à la signature de `checkAllowedUser()`/`checkAllowedUserPeek()` et le forwarder depuis les 4 proxys, comme déjà fait ailleurs. ⚠️ tester web ET natif (BUG 21/51). **PR sécu dédiée.**
...
- [ ] **MED — `contacts/action.ts:90-106` : `resourceName` non validé/encodé** — RECONFIRMÉ 3 juillet, toujours ouvert. Seul endpoint Google qui casse la baseline « IDs validés par regex ». Query-string injection dans l'URL People (bornée : compte de l'appelant, pas de cross-user). Fix : `^people\/[a-zA-Z0-9_-]+$` + `encodeURIComponent`.
...
- [ ] **MED — N-1 `aud` sur les 4 proxys IA principaux** : voir détail plus haut (7 juin, requalifié 3 juillet) — le fix doit toucher `checkAllowedUser()` lui-même, pas juste ses appelants. PR sécu dédiée (test web+natif).
- [ ] **MED — `ai/proxy.ts:271-276` fuite le body d'erreur upstream Anthropic** (nouveau 3 juillet) — seul des 6 proxys IA sans masquage ; un user authentifié peut lire l'état de la clé/compte owner (crédit, rate-limit) en déclenchant un 400/429 sur clé serveur. Fix : dupliquer le pattern des 5 autres proxys (`isByok` déjà dispo ligne 49).
- [ ] **LOW — `memory/action.ts:101-105` et `sheets/append.ts:30-33` : `err.message` renvoyé au client sur 500** (nouveau 3 juillet, même classe que le point précédent) — détails de schéma D1/Sheets exposés. Fix : message générique + `console.error`.
...
- [ ] **LOW — permissions Android sur-déclarées** (`READ_MEDIA_AUDIO/VIDEO`, `MODIFY_AUDIO_SETTINGS`) — RECONFIRMÉ 3 juillet avec preuve d'usage : aucune référence à ces permissions dans `src/` (galerie = photos only, pièces jointes = images+PDF only). À retirer avant Play Store. + `gemini-proxy` `model` non format-vérifié + `drive` `previousParents` non encodé + pas de timeout serveur→Google. Nettoyage avant Play Store.
- [ ] **LOW — `GoogleSignInPlugin.java` logue l'email complet + `server_client_id` sans garde `BuildConfig.DEBUG`** (nouveau 3 juillet, `:40,81,87`) — PII en logcat sur build release. Fix : `if (BuildConfig.DEBUG) Log.d(...)`, retirer l'email.
- [ ] **LOW (fonctionnel, pas une faille) — `public/_headers:5` : `Permissions-Policy: geolocation=()` bloque potentiellement la géoloc web/PWA** (nouveau 3 juillet) — `camera`/`microphone` autorisent `self`, mais `geolocation` a une allowlist vide alors que `location.ts:getBestFixWeb` (BUG 55) appelle `navigator.geolocation` directement sur le chemin web. Capacitor natif non affecté (plugin natif, pas ce header). Fix : `geolocation=(self)`.
