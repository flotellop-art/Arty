...
Dernier audit : **3 juillet 2026** (3 agents : backend Opus, crypto+auth Opus, frontend Sonnet + vérif terrain fichier:ligne sur les findings clés — voir résumé ci-dessous). Précédent : 14 juin 2026. Avant : 7 juin 2026, 4 mai 2026 (PR #127 + #128).

> **Corrections appliquées le 3 juillet 2026 (même session, sur validation utilisateur)** :
> tous les items MED/LOW nouveaux de l'audit ci-dessous ont été corrigés — `checkAllowedUser()`/
> `checkAllowedUserPeek()` valident maintenant `aud` (fix précédé de 2 agents challengers Opus,
> RÈGLE 7, cf. détail plus bas), `ai/proxy.ts` masque l'erreur upstream Anthropic hors BYOK,
> `contacts/action.ts` valide `resourceName`, `memory/action.ts`/`sheets/append.ts` ne fuient
> plus `err.message`, `gemini-proxy.ts` valide le format de `model`, permissions Android
> média inutilisées retirées, logs PII natifs supprimés, `geolocation=(self)` corrigé.
> 502/502 tests passent, `tsc --noEmit` propre. Détail par item dans les sections ci-dessous.

> **MAJ 3 juillet 2026** — Audit complet `/audit-secu` (3 agents Opus/Opus/Sonnet), routine
> mensuelle. **Verdict : aucun CRIT, aucun HIGH exploitable.** Les 3 HIGH backend et les
> corrections du lot « priorités sûres » du 14 juin restent tous vérifiés en place.
> **4 items retirés du TODO après vérification terrain** : `weather.ts` utilise open-meteo
> (gratuit, sans clé) — pas d'abus de la clé Google Maps possible via cet endpoint, l'item
> TODO était erroné ; `geo/reverse.ts` est désormais plafonné par un cap journalier par email
> (`planSubjectToOwnerApiCap`/`consumeOwnerApiQuota`) — reclassé de MED à mitigé ; le pont
> `computer/relay` + `local/computer-use-server.js` a déjà une validation stricte des
> coordonnées/touches (`computer-use-safety.js`, PR #303 du 28 juin) — l'item LOW était
> périmé ; `/api/license/activate` sans token Google est une **décision assumée et
> documentée dans le code** (PR #303/#304 du 28 juin), pas un bug — ⚠️ ces 2 derniers points
> avaient été reconduits par erreur dans une première passe de ce même audit (contexte de
> prompt basé sur l'état du 14 juin, sans checker le `git log` récent) puis corrigés après
> relecture du code juste avant le commit.
> **1 MED nouveau confirmé par grep+lecture** : `checkAllowedUser()` (`_lib/checkAllowedUser.ts:266-271`)
> n'a pas de paramètre `expectedAud` du tout et appelle `verifyGoogleUser(request)` sans
> audience — les 4 proxys IA principaux (`ai/proxy.ts:63`, `gemini-proxy.ts:56`,
> `mistral-proxy.ts:56`, `openai-proxy.ts:56`) passent tous par ce chemin non durci, alors que
> les endpoints périphériques (image-gen, tts, whisper, voxtral, memory-extract) sont déjà
> corrects avec `verifyGoogleUser(request, env.GOOGLE_CLIENT_ID)`. Le N-1 `aud` n'est donc PAS
> fermé, contrairement à ce qu'une lecture rapide pourrait suggérer — il est fermé sur le
> mauvais périmètre (le moins cher) et ouvert sur le plus cher. **1 MED nouveau** :
> `ai/proxy.ts:271-276` est le seul proxy IA (sur 6) qui ne masque pas le body d'erreur
> upstream — Gemini/Mistral/OpenAI/TTS/Whisper/Voxtral le font tous. Détail complet dans les
> sections ci-dessous.
...
> - **Sanitize markdown durci** : wildcard `data*` → allowlist explicite (props camelCase
>   hast), `data:` retiré de `protocols.src` (XSS SVG WebView). Tests de non-régression ajoutés.
> - **Expiration des abonnements** (`checkAllowedUser.ts` + `subscription/status.ts`) : un abo
>   `cancelled` au-delà de `current_period_end` ne donne plus accès (fuite de revenu si le
>   webhook `expired` est manqué). `active` reste inconditionnel (pas de lockout sur renew lag).
> - **PII en logs** : `googleAuth.ts` ne logge plus le body complet de la réponse refresh.
> - **Pro = BYOK** (PR #287, même session) : un compte Pro sans BYOK ne récupère plus la clé serveur.
>
...
- [x] **N-1 (MED) — `checkAllowedUser()` ne validait pas `aud` sur les 4 proxys IA principaux — CORRIGÉ le 3 juillet.** `checkAllowedUser()` et `checkAllowedUserPeek()` (`_lib/checkAllowedUser.ts:252,270`) appellent désormais `verifyGoogleUser(request, env.GOOGLE_CLIENT_ID)` — aucun changement de signature nécessaire (`env` déjà en scope). Couvre d'un coup les 4 proxys chat + tous les endpoints `checkAllowedUserPeek` (weather, geo/reverse, quota status/month, search/web, fetch/url). Validé par 2 agents challengers Opus avant implémentation (RÈGLE 7) : preuve que le pattern est déjà en prod sans casse sur tts/whisper/voxtral (endpoints natifs), fail-safe `null`=indéterminé confirmé sûr par construction (le même `GOOGLE_CLIENT_ID` sert à émettre ET vérifier le token natif via `auth/token.ts`). 502/502 tests passent. Étendre aux ~12 endpoints qui appellent `verifyGoogleUser` directement (memory/calendar/sheets/gmail/share/billing/contacts/wordpress/wallet/drive/account) a été **différé** (agent : bénéfice marginal, ce sont des relais du token de l'utilisateur lui-même vers ses propres données/APIs Google, pas la clé owner).
- [x] **N-2 (MED) — fuite status/body upstream `search/web.ts`** : VÉRIFIÉ CORRIGÉ le 14 juin — `web.ts` renvoie déjà `{error:'Search failed'}` 502, l'`err.message` upstream n'est pas propagé (le TODO était périmé). La même classe sur transcription a été faite en PR #269. **Statut au 3 juillet** : `wordpress/action.ts` est **largement corrigé** — messages génériques partout (`'Failed to list posts'`, `'Create failed'`, …), seul le `r.status` upstream (négligeable) est forwardé. `contacts/action.ts` propage encore `err.error.message` Google brut (l.50,83,109) — **requalifié LOW** (c'est le token de l'appelant sur ses propres données, pas la clé owner). **Nouveau (3 juillet, MED)** : `ai/proxy.ts:271-276` est le seul des 6 proxys IA qui ne masque PAS le body d'erreur upstream Anthropic (les 5 autres le font) — un user authentifié peut lire l'état de la clé/compte owner (crédit, rate-limit) en déclenchant un 400/429 sur clé serveur. Fix : dupliquer le pattern des 5 autres (masquer sauf en BYOK, `isByok` déjà disponible ligne 49). **Nouveau (3 juillet, LOW)** : même classe sur `memory/action.ts:101-105` et `sheets/append.ts:30-33` (`err.message` D1/Sheets renvoyé brut sur 500) — message générique + `console.error`.
...
**Nouveaux findings résiduels (audit 14 juin) — statut au 3 juillet** :
- [x] **MED — `contacts/action.ts` : `resourceName` non validé — CORRIGÉ le 3 juillet.** Regex `^people\/[a-zA-Z0-9_-]+$` ajoutée sur `resourceName` avant usage dans les URLs People API (`handleUpdate`). Pas d'`encodeURIComponent` supplémentaire (aurait cassé le `/` littéral du path Google) — la regex à elle seule garantit des caractères sûrs, c'est la sanitisation.
- [x] **`/api/license/activate` sans token Google — DÉCISION ASSUMÉE (PR #303/#304, 28 juin), pas un bug ouvert.** Correction du 3 juillet : les agents d'audit avaient reconduit cet item comme MED sur la base du dernier état connu (14 juin), mais le code contient depuis le 28 juin une décision explicite écrite (`activate.ts`, commentaire au-dessus de la requête D1) : un gate token Google était contournable (l'attaquant omet juste le header), et deux des 3 modes de login n'ont pas de token Google — un soft-require créait de la friction réelle pour une protection illusoire. La barrière reste la paire secrète `license_key` (~32 chars) + `email`, et Pro=BYOK (PR #287) fait qu'activer une licence ne donne plus accès aux clés IA serveur owner. Retiré du TODO — ne pas re-signaler sauf changement du modèle de risque.
- [ ] **MED — sel PBKDF2 global + `KEY_CHECK_KEY`/`VERSION_KEY` globaux** (`crypto.ts:14,80-88`) — RECONFIRMÉ 3 juillet + nuance nouvelle : ces clés localStorage ne sont PAS préfixées par `userId` (contrairement à `scopedStorage`) → sur appareil multi-comptes sans BYOK, couplage de données (même passphrase publique + même sel → même clé AES, blobs mutuellement déchiffrables) ET `selfTestCrypto()` (garde BUG 47) devient inopérant après un switch de compte (teste contre le check-blob d'un autre user → toujours `false`, conservateur mais silencieux). Borné (sandbox OS). À regrouper avec le chantier IndexedDB `CryptoKey` non-extractible.
- [x] **MED — météo `weather.ts`** — **FAUX POSITIF, infirmé le 3 juillet.** Utilise open-meteo (gratuit, sans clé) pour géocodage ET forecast, pas Google Maps. Seule exposition = relais bande-passante, déjà gardé par `checkAllowedUserPeek`. Retiré du TODO.
- [x] **MED → mitigé — géoloc `geo/reverse.ts`** — Utilise bien la clé Google Maps, mais un cap journalier par email (`planSubjectToOwnerApiCap` + `consumeOwnerApiQuota`) plafonne désormais l'abus sur comptes non-payants. Reclassé de MED-ouvert à mitigé ; clé en query-string reste une exception legacy documentée (l'API Geocoding ignore le header).
- [x] **MED — N-1 `aud` sur les 4 proxys IA principaux — CORRIGÉ le 3 juillet.** Voir détail plus haut (7 juin, fix final dans `checkAllowedUser()`/`checkAllowedUserPeek()`).
- [x] **MED — `ai/proxy.ts:271-276` fuite le body d'erreur upstream Anthropic — CORRIGÉ le 3 juillet.** Masquage `if (!isByok)` (log serveur + `{error:'AI service error'}`, status préservé), passthrough conservé en BYOK (le client a besoin du message verbatim pour distinguer clé invalide/prompt trop long — `anthropicClient.ts`). Le `catch` (erreur réseau) est aussi généricisé (`err.message` ne fuitait qu'une exception JS, mais même classe). Validé par agent challenger Opus avant implémentation.
- [x] **LOW — `memory/action.ts:101-105` et `sheets/append.ts:30-33` : `err.message` renvoyé au client sur 500 — CORRIGÉ le 3 juillet.** Message générique + `console.error` côté serveur sur les deux fichiers.
- [x] **`computer/relay` + `local/computer-use-server.js` — DÉJÀ CORRIGÉ (PR #303, 28 juin), pas un bug ouvert.** Correction du 3 juillet : `local/computer-use-safety.js` (nouveau fichier, testé en isolation) valide désormais les coordonnées (entiers bornés, rejette tout ce qui n'est pas numérique → empêche l'injection PowerShell via `click`) et normalise `key` via un `KEY_MAP` fermé (`normalizeKey`, rejette toute touche hors liste). `computer-use-server.js:82` dispatch sur un `switch(action)` à cases fixes (`screenshot`/`open_app`/`click`/`type`/`scroll`/`key`) — pas de `default` qui exécute une action arbitraire. `relay.ts` transmet toujours `action`/`params` sans allowlist à ce niveau, mais l'exécution réelle (côté PC owner) est bornée. Retiré du TODO.
- [x] **LOW — permissions Android sur-déclarées — CORRIGÉ le 3 juillet.** `READ_MEDIA_AUDIO`/`READ_MEDIA_VIDEO` retirées d'`AndroidManifest.xml` (confirmé inutilisées, preuve d'usage à l'audit). `MODIFY_AUDIO_SETTINGS` **conservée** sur avis agent challenger : permission "normal" sans prompt, probablement requise par le routing micro (Whisper/`RECORD_AUDIO`), bénéfice sécu nul vs risque de régression audio. `gemini-proxy` `model` désormais validé par regex `^[a-zA-Z0-9.-]+$` avant interpolation URL (seul proxy IA à mettre le modèle dans l'URL plutôt que le body). `drive` `previousParents` et timeout serveur→Google : non traités (IDs de confiance venant de la réponse Google, non exploitable) — restent hors scope.
- [x] **LOW — `GoogleSignInPlugin.java` loguait l'email complet + `server_client_id` sans garde — CORRIGÉ le 3 juillet.** Les 2 lignes `Log.d` contenant du PII (email, `server_client_id`) supprimées. Pas de garde `BuildConfig.DEBUG` (agent challenger : `buildFeatures.buildConfig` non activé dans `build.gradle`, un `if (BuildConfig.DEBUG)` aurait risqué de ne pas compiler) — suppression pure retenue, plus simple et robuste. Les logs de flux non sensibles (`signIn() called`, `ActivityResult received`, etc.) conservés (hors périmètre du finding).
- [x] **LOW (fonctionnel, pas une faille) — `public/_headers:5` : `Permissions-Policy: geolocation=()` — CORRIGÉ le 3 juillet.** Passé à `geolocation=(self)`. Confirmé par agent challenger : bug fonctionnel actif (bloquait déjà la géoloc PWA web), aucune régression (débloque une feature cassée).
- [ ] **LOW — PKCE OAuth toujours absent** (`googleAuth.ts:buildOAuthUrl`, `token.ts`) — RECONFIRMÉ 3 juillet, PR 2 toujours planifiée non faite. Impact atténué : client confidentiel + state CSRF déjà en place.
...
- [x] **Sanitize markdown** — `data*` → allowlist explicite (props camelCase) + `data:` retiré de `protocols.src` (`MarkdownRenderer.tsx`). Tests ajoutés.
- [x] **Expiration des abonnements** — `checkAllowedUser.ts` + `subscription/status.ts` (cancelled au-delà de `current_period_end` → plus d'accès ; active inconditionnel).
- [x] **PII en logs** — `googleAuth.ts` ne logge plus le body complet de la réponse refresh.
