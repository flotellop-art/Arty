...
describe('candidate isolated runtime, deliberately disabled in production', () => {
  ...
  it('isolated account erasure keeps authority and all bytes for a NEW cold document, retiring all private writers', async () => {
    ...
    expect(await wipeLocalAccount()).toBe('reload-required')
    ...
    for (const slot of HISTORY_SLOTS) expect(localStorage.getItem(key('a', slot))).toBe('a-bytes')
    ...
    expect(await db.countFromIndex('projects', 'owner', 'a')).toBe(1)
    expect(await db.countFromIndex('projects', 'owner', 'b')).toBe(1)
    expect(await db.get('meta', ['erasing', 'a'])).toMatchObject({ localOnly: true })
    expect(await fdb.get('files', 'a-file')).toBeDefined(); expect(await fdb.get('files', 'b-file')).toBeDefined()
    db.close(); fdb.close()
    await expect(enter('a')).rejects.toThrow(); await expect(enter('b')).rejects.toThrow()
    expect(fetch).not.toHaveBeenCalled()
  })
