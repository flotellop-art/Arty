...
describe('TikTok in a real conversation hook, simulated providers', () => {
  ...
  it('shows the failed video stage without generating or retrying after a network rejection', async () => {
    const network = vi.fn().mockRejectedValue(new TypeError('Failed to fetch')); vi.stubGlobal('fetch', network)
    const hook = setup()
    await act(async () => { await hook.result.current.sendMessage(url, conv.id) })
    expect(hook.result.current.error).toContain('résultat de la lecture TikTok')
    expect(conv.messages.every(m => !m.videoAnalysis)).toBe(true)
    expect(streamMessage).not.toHaveBeenCalled()
    expect(network).toHaveBeenCalledOnce()
  })
  it('keeps completed observations when the subsequent chat fails on the network', async () => {
    const network = vi.fn(async () => response()); vi.stubGlobal('fetch', network)
    const hook = setup()
    await act(async () => { await hook.result.current.sendMessage(url, conv.id) })
    act(() => vi.mocked(streamMessage).mock.calls[0]![3](new TypeError('Failed to fetch')))
    expect(hook.result.current.error).toContain('réponse du serveur')
    expect(conv.messages[0]!.videoAnalysis?.text).toBe(observation)
    expect(network).toHaveBeenCalledOnce()
  })
})
