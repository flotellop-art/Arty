...
const mocks = vi.hoisted(() => ({ inspect: vi.fn(), unlockOrJoin: vi.fn(), receive: vi.fn(), prepareApply: vi.fn(), applyReceived: vi.fn(), close: vi.fn(), create: vi.fn(),
  resume: vi.fn(), pendingStatus: vi.fn(), reconcilePending: vi.fn(), abort: vi.fn(), eraseLocal: vi.fn(), cold: vi.fn(), enabled: true, phase: 'prepared' }))
...
vi.mock('../../services/workspaceWriter/runtime', () => ({ documentWorkspaceSignal: new AbortController().signal,
  getDocumentStorageLayout: () => ({ kind: 'isolated-v1', projects: { version: 2 } }), workspaceAdmission: { getSyncApplyRecovery: () => ({ apply: { phase: mocks.phase } }) } }))
...
beforeEach(() => {
  vi.clearAllMocks(); mocks.enabled = true; mocks.phase = 'prepared'
