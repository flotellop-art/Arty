...
describe('real handlers and middleware on local D1/R2/workerd', () => {
  ...
  it('rejects legacy erasure without mutation, then cleans only the captured incarnation and defeats old replays', async () => {
    ...
    const deletion = await erase(); const started = await deletion.post(); expect(started.status).toBe(202)
    expect(await started.json()).toEqual({ protocol: 1, operationId: deletion.operationId, subjectHash: deletion.intent.subjectHash, status: 'cleanup-pending' })
    expect((await h.mf.dispatchFetch(p.url('object'), { headers: auth })).status).toBe(410)
    const before = await h.db.prepare('SELECT * FROM workspace_sync_vaults_v1').all(), authCalls = h.authCalls.length
    expect(await (await deletion.get()).json()).toEqual({ protocol: 1, operationId: deletion.operationId, subjectHash: deletion.intent.subjectHash, status: 'cleanup-pending' })
    expect((await h.db.prepare('SELECT * FROM workspace_sync_vaults_v1').all()).results).toEqual(before.results)
    expect(h.authCalls).toHaveLength(authCalls)
