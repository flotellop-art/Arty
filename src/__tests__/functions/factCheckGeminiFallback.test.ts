...
describe('endpoint fact-check, secours fournisseur Gemini', () => {
  ...
  it('sert réellement Gemini quand Anthropic refuse la requête', async () => {
    ...
    expect(String(fetchMock.mock.calls[1]?.[0])).toContain('/gemini-3.8-flash:generateContent')
  ...
  it('normalise la réponse Gemini 3.8 et active Google Search pour la passe approfondie', async () => {
  ...
  it('essaie Gemini 3.6 si Gemini 3.8 est indisponible', async () => {
    fetchMock
    ...
    expect(result.payload?.model).toBe('gemini-3.6-flash')
    expect(fetchMock).toHaveBeenCalledTimes(2)
  ...
  it.each(['end_turn', 'max_tokens', 'pause_turn', undefined])('atteste la fin Anthropic uniquement pour end_turn (%s)', async stopReason => {
    ...
    }), env: { DB: { prepare: () => ({ run: async () => ({ success: true }) }) }, ANTHROPIC_API_KEY: 'test-key' } } as never)
