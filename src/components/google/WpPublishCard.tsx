...
export function WpPublishCard({
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden my-2">
      <div className="bg-purple-50 px-4 py-3 border-b border-gray-100 flex items-center justify-between">
      ...
      <div className="px-4 py-3 space-y-2">
        <div>
          <label className="text-xs text-gray-500 block mb-0.5">Titre</label>
          <p className="text-sm font-medium text-bubble-user bg-gray-50 rounded-lg px-3 py-2">
            {title}
          </p>
        </div>
        <div>
          <label className="text-xs text-gray-500 block mb-0.5">Extrait du contenu</label>
          <p className="text-sm text-bubble-user bg-gray-50 rounded-lg px-3 py-2 line-clamp-4 leading-relaxed">
      ...
      <div className="flex gap-2 px-4 py-3 border-t border-gray-100">
        ...
        <button
          ...
          className={`flex-1 py-2 rounded-lg text-white text-sm font-medium transition-colors disabled:opacity-50 ${
            confirmed ? 'bg-red-500 hover:bg-red-600' : 'bg-accent hover:bg-accent/90'
          }`}
        >
