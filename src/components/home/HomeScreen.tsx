  ...
  return (
    <div className="flex flex-col h-full bg-theme-bg text-theme-ink">
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {/* Spark — proactive brief launcher, one click through the normal
            chat pipeline (routes to Claude + Gmail/Calendar tools). No
            autonomous writes: the prompt asks for confirmation before any action. */}
        {googleAuth.isConnected && (
          <div className="px-6 pt-7 max-w-3xl">
            <button
              onClick={() => onSend(t('home.spark.prompt'))}
              disabled={isStreaming}
              aria-label={t('home.spark.label')}
            ...
            >
              ...
              <span className="flex-1 min-w-0">
                <span className="block font-display font-medium text-[16px] leading-tight">{t('home.spark.label')}</span>
                <span className="block font-sans text-[11px] leading-snug opacity-75 mt-0.5">{t('home.spark.description')}</span>
              </span>
