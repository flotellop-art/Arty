...
export function ProjectReviewDialog({ request, onAnswer }: {
  ...
  const [mode, setMode] = useState<'search' | 'overview'>('search')
  const [ids, setIds] = useState<string[]>(() => request.kind === 'select' ? request.project.documents.map(d => d.id) : [])
  useEffect(() => {
    ...
    const onKey = (event: KeyboardEvent) => {
      ...
      if (event.key === 'Tab') {
        const nodes = [...(root.current?.querySelectorAll<HTMLElement>('button:not(:disabled),input:not(:disabled),select,details>summary') ?? [])]
    ...
    <div ref={root} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby="project-review-title" className="bg-theme-bg text-theme-ink border border-theme-border rounded-xl max-w-3xl w-full max-h-[90dvh] overflow-y-auto p-5 space-y-4">
      ...
      {request.kind === 'select' ? <>
        ...
        <label className="block">{label('Mode de sélection locale', 'Local selection mode')}
          <select className="block w-full bg-theme-bg border p-2" value={mode} onChange={e => setMode(e.target.value as typeof mode)}>
        ...
        <fieldset><legend>{label('Documents à examiner', 'Documents to examine')}</legend>
          {request.project.documents.map(doc => <label key={doc.id} className="flex gap-2 py-2 break-all"><input type="checkbox" checked={ids.includes(doc.id)} onChange={e => setIds(values => e.target.checked ? [...values, doc.id] : values.filter(id => id !== doc.id))} />{doc.name}</label>)}
        </fieldset>
        <p className="text-sm">{label('La recherche est locale et lexicale. Aucun extrait trouvé ne déclenche automatiquement une synthèse globale. L’étape suivante montre les extraits et le destinataire.', 'Search is local and lexical. No matches never trigger an automatic overview. The next step shows the excerpts and recipient.')}</p>
        <div className="flex gap-3"><button className={button} onClick={() => onAnswer(null)}>{label('Annuler', 'Cancel')}</button><button className={button} disabled={request.project.documents.length > 0 && ids.length === 0} onClick={() => onAnswer({ mode, documentIds: ids })}>{label('Préparer l’aperçu', 'Prepare preview')}</button></div>
      </> : <>
        ...
        <details><summary>{label('Question effective et consignes principales', 'Effective question and main instructions')}</summary><p className="text-xs">{label('Les clients ajoutent aussi leurs règles fixes et la date, couvertes par la réserve système.', 'Clients also add their fixed rules and date, covered by the system reserve.')}</p><pre className="whitespace-pre-wrap break-words text-xs">{request.question}{'\n\n'}{request.systemPrompt}</pre></details>
        ...
        <div className="flex gap-3"><button className={button} onClick={() => onAnswer(null)}>{label('Annuler, garder mon texte', 'Cancel, keep my text')}</button><button className={button} onClick={() => onAnswer(true)}>{request.context?.noHit ? label('Envoyer sans nouvel extrait', 'Send without new excerpts') : label('Confirmer cet envoi', 'Confirm this request')}</button></div>
      </>}
    </div>
  </div>
}
