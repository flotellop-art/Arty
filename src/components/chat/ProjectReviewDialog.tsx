...
export function ProjectReviewDialog({ request, onAnswer }: {
  ...
  const reply = request.kind === 'confirm' ? request.clientReply : undefined
  const replyTones = { professional: label('Professionnel', 'Professional'), warm: label('Chaleureux', 'Warm'), firm: label('Ferme et courtois', 'Firm and courteous') }
  ...
  return <div className="fixed inset-0 z-[100] bg-black/60 flex items-center justify-center p-3">
    <div ref={root} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby="project-review-title" className="bg-theme-bg text-theme-ink border border-theme-border rounded-xl max-w-3xl w-full max-h-[90dvh] overflow-y-auto p-5 space-y-4">
      <h2 id="project-review-title" className="text-xl">{reply ? label('Réponse client — avant l’appel IA', 'Client reply — before the AI request') : label('Contexte du projet — avant envoi', 'Project context — before sending')}</h2>
      {request.kind === 'select' ? <>
        ...
        {reply && <section className="border border-theme-border rounded p-3 space-y-3">
          <p>{label('Ces champs seront transmis à l’IA, pas au client. Aucun envoi ni brouillon en messagerie.', 'These fields will be sent to the AI, not the client. No sending or mailbox draft.')}</p>
          {([['request', label('Demande du client', 'Client request')], ['facts', label('Faits autorisés pour la réponse', 'Facts allowed in the reply')], ['objective', label('Objectif de la réponse', 'Reply objective')], ['tone', label('Ton', 'Tone')]] as const).map(([key, title]) => <div key={key}><h3>{title}</h3><pre className="whitespace-pre-wrap break-words text-sm">{key === 'tone' ? replyTones[reply.tone] : reply[key] || (key === 'facts' ? label('Aucun fait complémentaire déclaré.', 'No additional facts declared.') : '')}</pre></div>)}
          {reply.noAdditionalFacts && <p>{label('L’absence de faits complémentaires a été confirmée.', 'The absence of additional facts was confirmed.')}</p>}
        </section>}
        ...
        {!request.context && <p>{reply ? label('Nouveau fil : aucun historique, bibliothèque, fichier ou contexte de compte connecté joint.', 'New chat: no history, library, file or connected-account context attached.') : label('Bibliothèque non jointe ; historique toujours transmis. Les anciennes sources ne sont pas relues.', 'No library attached; history is still sent. Previous sources are not reread.')}</p>}
        {request.context && <>
        ...
        <div className="flex gap-3"><button className={button} onClick={() => onAnswer(null)}>{label('Annuler, garder mon texte', 'Cancel, keep my text')}</button><button className={button} onClick={() => onAnswer(true)}>{reply ? label('Confirmer l’appel IA', 'Confirm the AI request') : request.comparisonModels ? label('Confirmer les deux appels', 'Confirm both requests') : request.context?.noHit ? label('Envoyer sans nouvel extrait', 'Send without new excerpts') : label('Confirmer cet envoi', 'Confirm this request')}</button></div>
      </>}
    </div>
  </div>
}
