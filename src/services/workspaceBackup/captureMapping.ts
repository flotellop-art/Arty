...
export function mapCapturedConversation(source: Conversation): BackupConversation {
  ...
  const fact: Mapper = v => shape(v, ['overallConfidence', 'claims', 'modelLabel', 'checkedAt'], ['status', 'originalContent', 'appliedCorrections', 'limitations', 'coverage', 'progress'], {
    ...
    progress: p => shape(p, ['phase', 'batchesDone', 'batchesTotal', 'identified', 'accepted']),
  })
  const file: Mapper = v => {
